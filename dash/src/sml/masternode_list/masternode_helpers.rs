...
impl MasternodeList {
    pub fn masternode_for(&self, pro_reg_tx_hash: &ProTxHash) -> Option<&QualifiedMasternodeListEntry> {
        self.masternodes.get(pro_reg_tx_hash)
    }

    pub fn has_valid_masternode(&self, pro_reg_tx_hash: &ProTxHash) -> bool {
        self.masternodes.get(pro_reg_tx_hash).map_or(false, |node| node.masternode_list_entry.is_valid)
    }
            ...
            .any(|node| {
                match node.masternode_list_entry.service_address.ip() {
                    IpAddr::V4(ipv4) => {
                        ...
                        address[..4] == ipv4_bytes && node.masternode_list_entry.service_address.port() == port
                    }
                    IpAddr::V6(ipv6) => {
                        let ipv6_bytes = ipv6.octets();
                        address == ipv6_bytes && node.masternode_list_entry.service_address.port() == port
                    }
                }
            })
    }
    ...
    pub fn masternode_by_pro_reg_tx_hash(&self, registration_hash: &ProTxHash) -> Option<QualifiedMasternodeListEntry> {
    ...
    pub fn provider_tx_ordered_hashes(&self) -> Vec<ProTxHash> {
        ...
        vec.sort_by(|hash1, hash2| if reverse_cmp_sup(hash1.to_byte_array(), hash2.to_byte_array()) { Ordering::Greater } else { Ordering::Less });
        vec
    }
