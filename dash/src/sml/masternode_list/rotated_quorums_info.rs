...
impl MasternodeList {
    ...
    pub fn usage_info<'a>(&'a self, previous_quarters: [&Vec<Vec<&'a QualifiedMasternodeListEntry>>; 3], quorum_count: usize) -> (Vec<&'a QualifiedMasternodeListEntry>, Vec<&'a QualifiedMasternodeListEntry>, Vec<Vec<&'a QualifiedMasternodeListEntry>>) {
        let mut used_masternodes = Vec::<&QualifiedMasternodeListEntry>::new();
        let mut used_indexed_masternodes = vec![Vec::<&QualifiedMasternodeListEntry>::new(); quorum_count];
                ...
                if let Some(quarter_nodes) = quarter.get(i) {
                    for node in quarter_nodes {
                        ...
                        if self.has_valid_masternode(&hash.reverse()) {
                            ...
                            if !used_indexed_masternodes[i].iter().any(|m| m.masternode_list_entry.pro_reg_tx_hash == hash) {
        ...
        let unused_at_h_masternodes = self.masternodes.values()
            .filter(|mn| mn.masternode_list_entry.is_valid && !used_masternodes.iter().any(|node| mn.masternode_list_entry.pro_reg_tx_hash == node.masternode_list_entry.pro_reg_tx_hash))
            .collect();
