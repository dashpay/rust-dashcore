...
impl MasternodeListEngine {
    ...
    /// Once a diff on `base` moved the tip, drops the lists out of the retention
    /// window, never `base`, and the state only they referenced.
    pub(super) fn prune_after_extending(&mut self, base: CoreBlockHeight) {
        let Some(tip) =
            self.latest_masternode_list().map(|list| list.known_height).filter(|tip| *tip > base)
        else {
            return;
        };
        let floor = tip.saturating_sub(self.retention_window()).min(base);
...
mod prune_tests {
    ...
    #[test]
    fn a_diff_that_moves_the_tip_drops_the_lists_below_the_window() {
        let floor = TIP + 1
    ...
    #[test]
    fn diffs_sharing_an_older_base_all_apply() {
        let base = TIP - 10_000;
        let mut engine = MasternodeListEngine::dummy_with_lists(&[base]);

        for (from, to) in [(base, TIP - 100), (base, TIP)] {
            engine.apply_diff(MnListDiff::dummy_between(from, to), Some(to), false, None).unwrap();
        }
        assert_eq!(heights(&engine), vec![base, TIP - 100, TIP]);

        engine
            .apply_diff(MnListDiff::dummy_between(TIP, TIP + 1), Some(TIP + 1), false, None)
            .unwrap();
        assert_eq!(heights(&engine), vec![TIP - 100, TIP, TIP + 1]);
    }
    ...
    fn pruning_drops_the_state_only_pruned_lists_referenced() {
        let old = 900_000;
        let kept = TIP - 100;
        let mut engine = MasternodeListEngine::dummy_with_lists(&[old, kept, TIP]);
        ...
        engine.prune_after_extending(kept);
        assert_eq!(heights(&engine), vec![kept, TIP]);
    ...
    fn prune_keeps_what_chain_locks_near_the_tip_verify_against() {
        ...
        engine.prune_after_extending(tip - 1);
