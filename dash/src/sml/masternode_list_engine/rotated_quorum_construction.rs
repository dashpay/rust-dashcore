...
impl MasternodeListEngine {
    ...
    fn masternode_list_entry_members_for_rotated_quorum(
        ...
        let q_h_m_3c = self.quorum_quarter_members_by_reconstruction_type(quorum_llmq_type, LLMQQuarterReconstructionType::Snapshot, work_block_height_for_index(3))?;
        // Reconstruct quorum members at h - 2c from snapshot
        let q_h_m_2c = self.quorum_quarter_members_by_reconstruction_type(quorum_llmq_type, LLMQQuarterReconstructionType::Snapshot, work_block_height_for_index(2))?;
        // Reconstruct quorum members at h - c from snapshot
        let q_h_m_c = self.quorum_quarter_members_by_reconstruction_type(quorum_llmq_type, LLMQQuarterReconstructionType::Snapshot, work_block_height_for_index(1))?;
        // Determine quorum members at new index
        let reconstruction_type = LLMQQuarterReconstructionType::New { previous_quarters:  [&q_h_m_c, &q_h_m_2c, &q_h_m_3c] };
        let last_quarter = self.quorum_quarter_members_by_reconstruction_type(quorum_llmq_type, reconstruction_type, work_block_height_for_index(0))?;
        ...
        match reconstruction_type {
            ...
            LLMQQuarterReconstructionType::Snapshot => {
                if let Some(snapshot) = self.known_snapshots.get(work_block_hash) {
                    let (used_masternodes, unused_masternodes) = masternode_list.used_and_unused_masternodes_for_quorum(quorum_llmq_type, quorum_modifier_type, snapshot, self.network);
                    Ok(Self::apply_skip_strategy_of_type(LLMQQuarterUsageType::Snapshot(snapshot.clone()), used_masternodes, unused_masternodes, quorum_modifier, quorum_count, quarter_size))
                } else {
        ...
        match skip_type {
            ...
            LLMQQuarterUsageType::New(mut used_indexed_masternodes) => {
                ...
                for i in 0..quorum_count {
                    let masternodes_used_at_h_indexed_at_i = used_indexed_masternodes.get_mut(i).expect("expected to get index i quorum used indexed masternodes");
