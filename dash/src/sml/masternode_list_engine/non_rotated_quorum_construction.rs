...
impl MasternodeListEngine {
    ...
    pub(in crate::sml::masternode_list_engine) fn find_non_rotated_masternodes_for_quorum(
        ...
        let (masternode_list, known_block_height) = self.masternode_list_and_height_for_block_hash_8_blocks_ago(&quorum.quorum_entry.quorum_hash)?;
        ...
        let masternodes : Vec<&QualifiedMasternodeListEntry> = masternode_list.valid_masternodes_for_quorum(quorum, quorum_modifier_type, self.network);
        Ok(masternodes)
    }
}