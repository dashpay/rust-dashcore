...
impl NetworkMessage {
    ...
    ///
    /// This is the inverse of [`Self::consensus_encode_payload`], suitable for use with
...
mod test {
    ...
    fn test_payload_round_trip(msg: &NetworkMessage) {
        ...
        let decoded = NetworkMessage::consensus_decode_payload(msg.cmd(), &encoded)
            .unwrap_or_else(|e| panic!("Failed to decode {} message: {}", msg.cmd(), e));
    ...
    fn test_encode_decode_empty_payload_messages() {
        ...
        for msg in &empty_payload_messages {
            ...
            let decoded = NetworkMessage::consensus_decode_payload(msg.cmd(), &[])
                .unwrap_or_else(|e| panic!("Failed to decode empty {} message: {}", msg.cmd(), e));
    ...
    fn test_headers_message_special_encoding() {
        ...
        // Test single header
        let single_header = NetworkMessage::Headers(vec![header]);
        ...
        let multi_headers = NetworkMessage::Headers(vec![header, header, header]);
