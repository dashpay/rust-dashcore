...
impl Decodable for CompressionFlags {
    fn consensus_decode<R: io::Read + ?Sized>(r: &mut R) -> Result<Self, crate::consensus::encode::Error> {
...
impl CompressedHeader {
    ...
    pub fn is_full(&self) -> bool {
        self.flags.has_prev_block_hash() && self.flags.has_full_timestamp() && self.flags.has_nbits()
    }
...
impl CompressionState {
    ...
    pub fn decompress(&mut self, compressed: &CompressedHeader) -> Result<Header, DecompressionError> {
        // Version
        ...
        let prev_blockhash = if let Some(hash) = compressed.prev_blockhash {
            ...
            self.prev_header
                .as_ref()
                .ok_or(DecompressionError::MissingPreviousHeader)?
                .block_hash()
        };
        
        // Timestamp
        let time = if let Some(offset) = compressed.time_offset {
            let prev_time = self.prev_header
                .as_ref()
                .ok_or(DecompressionError::MissingPreviousHeader)?
                .time;
...
impl fmt::Display for DecompressionError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            DecompressionError::InvalidVersionOffset => write!(f, "invalid version offset in compressed header"),
            DecompressionError::MissingPreviousHeader => write!(f, "previous header required for decompression"),
            DecompressionError::MissingTimestamp => write!(f, "timestamp missing in compressed header"),
        }
    }
}
