...
async fn test_load_filters_range() {
    ...
    let mut storage = DiskStorageManager::new(temp_dir.path().to_path_buf())
        .await
        .expect("Failed to create Disk storage");
    ...
    storage.store_filter(102, &[7, 8, 9]).await.expect("Failed to store filter at 102");
    storage.store_filter(103, &[10, 11, 12]).await.expect("Failed to store filter at 103");

    let filters = storage.load_filters(100..104).await.expect("Failed to load filters");

    assert_eq!(filters.len(), 4, "Should load 4 filters");
    assert_eq!(filters[0], vec![1, 2, 3], "Filter at 100 mismatch");
    assert_eq!(filters[1], vec![4, 5, 6], "Filter at 101 mismatch");
    assert_eq!(filters[2], vec![7, 8, 9], "Filter at 102 mismatch");
    assert_eq!(filters[3], vec![10, 11, 12], "Filter at 103 mismatch");
}
