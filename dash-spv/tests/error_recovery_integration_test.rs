        ...
        if let Some(fail_height) = *self.fail_at_height.read().await {
            if height >= fail_height {
                return match &*self.failure_type.read().await {
                    FailureType::WriteFailure => Some(StorageError::WriteFailed(
                        format!("Simulated write failure at height {}", height)
                    )),
                    FailureType::ReadFailure => Some(StorageError::ReadFailed(
                        format!("Simulated read failure at height {}", height)
                    )),
                    FailureType::Corruption => Some(StorageError::Corruption(
                        format!("Simulated corruption at height {}", height)
                    )),
                    FailureType::DiskFull => Some(StorageError::WriteFailed(
...
async fn test_recovery_from_network_interruption_during_header_sync() {
    ...
    let storage = Arc::new(RwLock::new(
        DiskStorageManager::new(storage_path).await.unwrap()
    ));
    ...
    while current_height < target_height && recovery_count < max_recoveries {
        ...
        loop {
            if interruptor.should_interrupt().await {
                ...
                if let dash_spv::sync::sequential::recovery::RecoveryStrategy::Retry { delay } = strategy {
...
async fn test_recovery_from_validation_errors() {
    ...
    for (i, val_error) in validation_errors.iter().enumerate() {
        ...
        match strategy {
            dash_spv::sync::sequential::recovery::RecoveryStrategy::RestartPhase { checkpoint } => {
                ...
                eprintln!("Validation error '{}' triggers restart from height {}", 
...
async fn test_concurrent_error_recovery() {
    ...
    for (task_id, error_category, strategy) in results {
        eprintln!("Task {} with {} error got strategy: {:?}", 
                 task_id, error_category, strategy);
...
async fn test_recovery_statistics_tracking() {
    ...
    for (i, (error, _expected_success)) in scenarios.iter().enumerate() {
        ...
        let strategy = recovery_manager.determine_strategy(&phase, error);
        let _ = recovery_manager.execute_recovery(
            &mut phase, 
            strategy, 
            error,
            &mut network, 
...
impl dash_spv::network::NetworkManager for MockNetworkManager {
    ...
    async fn send_message(&mut self, _msg: dashcore::network::message::NetworkMessage) 
...
impl StorageManager for MockStorageManager {
    async fn store_header(&mut self, _height: u32, _header: &BlockHeader) 
        -> dash_spv::error::StorageResult<()> 
    {
        Ok(())
    }

    async fn get_header(&self, _height: u32) 
        -> dash_spv::error::StorageResult<Option<BlockHeader>> 
    {
        Ok(None)
    }

    async fn get_header_by_hash(&self, _hash: &BlockHash) 
    ...
    async fn get_headers_range(&self, _range: std::ops::Range<u32>) 
        -> dash_spv::error::StorageResult<Vec<BlockHeader>> 
    {
        Ok(vec![])
    }

    async fn store_filter_header(&mut self, _height: u32, _filter_header: &FilterHeader) 
        -> dash_spv::error::StorageResult<()> 
    {
        Ok(())
    }

    async fn get_filter_header(&self, _height: u32) 
        -> dash_spv::error::StorageResult<Option<FilterHeader>> 
    {
        Ok(None)
    }

    async fn get_filter_tip_height(&self) 
        -> dash_spv::error::StorageResult<Option<u32>> 
    {
        Ok(Some(0))
    }

    async fn store_chain_state(&mut self, _state: &dash_spv::types::ChainState) 
        -> dash_spv::error::StorageResult<()> 
    {
        Ok(())
    }

    async fn get_chain_state(&self) 
    ...
    async fn get_stats(&self) 
        -> dash_spv::error::StorageResult<dash_spv::storage::StorageStats> 
    {
    ...
    async fn get_utxos_by_address(&self, _address: &dashcore::Address) 
        -> dash_spv::error::StorageResult<Vec<(dashcore::OutPoint, dash_spv::wallet::Utxo)>> 
    {
        Ok(vec![])
    }

    async fn store_utxo(&mut self, _outpoint: &dashcore::OutPoint, _utxo: &dash_spv::wallet::Utxo) 
        -> dash_spv::error::StorageResult<()> 
    {
        Ok(())
    }

    async fn remove_utxo(&mut self, _outpoint: &dashcore::OutPoint) 
        -> dash_spv::error::StorageResult<Option<dash_spv::wallet::Utxo>> 
    {
        Ok(None)
    }

    async fn get_utxo(&self, _outpoint: &dashcore::OutPoint) 
        -> dash_spv::error::StorageResult<Option<dash_spv::wallet::Utxo>> 
    {
        Ok(None)
    }

    async fn get_all_utxos(&self) 
        -> dash_spv::error::StorageResult<std::collections::HashMap<dashcore::OutPoint, dash_spv::wallet::Utxo>> 
    {
        Ok(std::collections::HashMap::new())
    }

    async fn store_mempool_state(&mut self, _state: &dash_spv::types::MempoolState) 
        -> dash_spv::error::StorageResult<()> 
    {
        Ok(())
    }

    async fn get_mempool_state(&self) 
        -> dash_spv::error::StorageResult<Option<dash_spv::types::MempoolState>> 
    {
        Ok(None)
    }

    async fn store_masternode_state(&mut self, _state: &dash_spv::storage::MasternodeState) 
        -> dash_spv::error::StorageResult<()> 
    {
        Ok(())
    }

    async fn get_masternode_state(&self) 
        -> dash_spv::error::StorageResult<Option<dash_spv::storage::MasternodeState>> 
    {
        Ok(None)
    }

    async fn store_terminal_block(&mut self, _block: &dash_spv::storage::StoredTerminalBlock) 
        -> dash_spv::error::StorageResult<()> 
    {
        Ok(())
    }

    async fn get_terminal_block(&self) 
        -> dash_spv::error::StorageResult<Option<dash_spv::storage::StoredTerminalBlock>> 
    {
        Ok(None)
    }

    async fn clear_terminal_block(&mut self) 
        -> dash_spv::error::StorageResult<()> 
    {
        Ok(())
    }
