...
async fn test_header_sync_manager_creation() {
    ...
    let _storage = MemoryStorageManager::new().await
...
async fn test_basic_header_sync_from_genesis() {
    ...
    storage.store_chain_state(&chain_state).await
        .expect("Failed to store initial chain state");
...
async fn test_header_sync_continuation() {
    ...
    storage.store_headers(&existing_headers).await
        .expect("Failed to store existing headers");
    ...
    storage.store_headers(&continuation_headers).await
...
async fn test_header_validation_modes() {
    ...
    {
        let config = ClientConfig::new(Network::Dash)
            .with_validation_mode(ValidationMode::None);
...
async fn test_header_batch_processing() {
    ...
    for batch_start in (0..total_headers).step_by(batch_size) {
        ...
        assert_eq!(
            ...
            "Tip height should be {} after batch {}-{}", expected_tip, batch_start, batch_end
        );
    }
...
async fn test_header_sync_edge_cases() {
    ...
    storage.store_headers(&empty_headers).await
        .expect("Should handle empty header batch");
    assert_eq!(storage.get_tip_height().await.unwrap(), None);
    
    // Test 2: Single header
    let single_header = create_test_header_chain(1);
    storage.store_headers(&single_header).await
        .expect("Should handle single header");
    assert_eq!(storage.get_tip_height().await.unwrap(), Some(0));
    
    // Test 3: Large batch
    let large_batch = create_test_header_chain_from(1, 5000);
    storage.store_headers(&large_batch).await
        .expect("Should handle large header batch");
...
async fn test_header_chain_validation() {
    ...
    storage.store_headers(&chain).await
        .expect("Failed to store header chain");
...
async fn test_header_sync_performance() {
    ...
    assert!(sync_duration < Duration::from_secs(5), 
            "Sync of {} headers took too long: {:?}", total_headers, sync_duration);
    ...
    assert!(retrieval_duration < Duration::from_millis(100),
            "Header retrieval took too long: {:?}", retrieval_duration);
    
    info!("Header sync performance test completed: sync={}ms, retrieval={}ms", 
          sync_duration.as_millis(), retrieval_duration.as_millis());
}
...
async fn test_header_sync_error_handling() {
    ...
    let invalid_config = ClientConfig::new(Network::Dash)
        .with_validation_mode(ValidationMode::None); // Valid config for this test
