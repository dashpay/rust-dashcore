...
use dashcore::{BlockHash, Transaction, Network};
...
async fn test_smart_fetch_efficiency_metrics() {
    ...
    assert!(efficiency > 0.50, "Smart fetch should reduce requests by at least 50% (got {:.2}%)", efficiency * 100.0);
}
...
async fn test_smart_fetch_rotating_quorums() {
    ...
    for window in &windows {
        ...
        let expected_mining_start = window.cycle_start + params.signing_active_quorum_count + params.dkg_params.phase_blocks * 5;
...
async fn test_smart_fetch_platform_activation() {
    ...
    let has_platform_before = windows_before.values()
        .flat_map(|v| v.iter())
    ...
    let has_platform_after = windows_after.values()
        .flat_map(|v| v.iter())
        .any(|w| w.llmq_type == LLMQType::Llmqtype100_67);
