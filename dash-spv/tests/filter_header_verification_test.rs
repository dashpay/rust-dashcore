...
use dashcore::{
    hash_types::{FilterHeader, FilterHash},
...
impl NetworkManager for MockNetworkManager {
    ...
    fn should_ping(&self) -> bool { false }
...
fn calculate_expected_filter_header(filter_hash: FilterHash, prev_filter_header: FilterHeader) -> FilterHeader {
    let mut data = [0u8; 64];
...
async fn test_filter_header_verification_failure_reproduction() {
    ...
    filter_sync.start_sync_headers(&mut network, &mut storage).await
    ...
    let result = filter_sync.handle_cfheaders_message(
        first_cfheaders.clone(),
        &mut storage,
        &mut network
    ).await;
    
    match result {
        Ok(continuing) => println!("First batch processed successfully, continuing: {}", continuing),
    ...
    let last_stored_filter_header = storage.get_filter_header(first_batch_end).await
        .unwrap()
    ...
    let second_to_last_stored = storage.get_filter_header(second_to_last_height).await
        .unwrap()
        .expect("Second to last filter header should exist");
    
    let calculated_last_header = calculate_expected_filter_header(*last_filter_hash, second_to_last_stored);
...
async fn test_overlapping_batches_from_different_peers() {
    ...
    filter_sync.handle_cfheaders_message(
        initial_cfheaders,
        &mut storage,
        &mut network
    ).await.expect("Initial batch should succeed");
    ...
    let peer_a_prev_filter_header = storage.get_filter_header(999).await
        .unwrap()
    ...
    let result_a = filter_sync.handle_cfheaders_message(
        peer_a_cfheaders,
        &mut storage,
        &mut network
    ).await;
    ...
    let peer_b_prev_filter_header_stored = storage.get_filter_header(1499).await
        .unwrap()
    ...
    let result_b = filter_sync.handle_cfheaders_message(
        peer_b_cfheaders,
        &mut storage,
        &mut network
    ).await;
    ...
    match result_b {
        Ok(_) => {
            ...
            println!("  🎯 This is what we want - the system should be resilient to overlapping data!");
        }
        Err(e) => {
    ...
    println!("The fix is working - peers with different filter header views are handled gracefully.");
}
...
async fn test_filter_header_verification_overlapping_batches() {
    ...
    filter_sync.handle_cfheaders_message(
        ...
        &mut storage,
        &mut network
    ).await.expect("First batch should succeed");
    ...
    let correct_prev_filter_header = storage.get_filter_header(overlap_prev_height).await
        .unwrap()
...
async fn test_filter_header_verification_race_condition_simulation() {
    ...
    filter_sync.handle_cfheaders_message(
        ...
        &mut storage,
        &mut network
    ).await.expect("Batch A should succeed");
