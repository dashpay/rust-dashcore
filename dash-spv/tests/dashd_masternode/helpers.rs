...
/// Wait for a `MasternodeStateUpdated` event that carries a `QRInfoFeedSummary`
/// whose `stored_cycle_height` is above `min_cycle_height` and which reports
/// `all_freshly_validated()` (i.e. every rotated quorum went through fresh
/// validation and the rotation cycle was actually stored in
/// `rotated_quorums_per_cycle`). Use this when a test needs the SPV to have a
/// freshly-validated rotation cycle before proceeding — for example, when
/// preparing to verify a post-rotation InstantSend lock.
pub(super) async fn wait_for_mn_state_with_stored_cycle_above(
    ...
    loop {
        tokio::select! {
            _ = &mut timeout => {
                panic!(
                    "Timeout waiting for MasternodeStateUpdated carrying a freshly-validated rotation cycle above height {}",
                    min_cycle_height
                );
            }
            result = event_receiver.recv() => {
                match result {
                    Ok(SyncEvent::MasternodeStateUpdated {
                        height,
                        qr_info_summary: Some(ref s),
                    }) if s.all_freshly_validated()
                        && matches!(s.stored_cycle_height, Some(h) if h > min_cycle_height) =>
                    {
                        tracing::info!(
                            "MasternodeStateUpdated at height {} with freshly-validated stored_cycle_height={:?}",
                            height,
                            s.stored_cycle_height,
                        );
                        return height;
                    }
                    Ok(SyncEvent::MasternodeStateUpdated { height, qr_info_summary }) => {
                        tracing::debug!(
                            "MasternodeStateUpdated at height {} (waiting for stored cycle > {}, got {:?})",
                            height,
                            min_cycle_height,
                            qr_info_summary.as_ref().map(|s| (s.stored_cycle_height, s.freshly_validated_count, s.rotated_quorum_count)),
                        );
                        continue;
                    }
                    Ok(_) => continue,
