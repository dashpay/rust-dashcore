...
async fn test_instantsend_full_lifecycle() {
    ...
    for i in 0..NUM_TXS {
        ...
        wait_for_controller_islock(&mut ctx.mn_ctx, &txid, 60);
...
async fn test_instantsend_across_quorum_rotation() {
    ...
    ctx.mn_ctx.mine_dkg_cycle().expect("First DKG cycle should succeed");
    let post_first_height = wait_for_mn_state_event_above(
        &mut client_handle.sync_event_receiver,
        initial_height,
        SYNC_TIMEOUT,
    )
    ...
    wait_for_controller_islock(&mut ctx.mn_ctx, &pre_txid, 60);
    ...
    wait_for_controller_islock(&mut ctx.mn_ctx, &post_txid, 60);
    let _ = wait_for_instant_lock_received(
        &mut client_handle.sync_event_receiver,
        post_txid,
        true,
...
async fn test_instantsend_islock_arrives_before_tx() {
    ...
    // Same rationale as `wait_for_controller_islock` in `setup.rs`: one
    // initial mocktime bump to wake the scheduler, then poll on real time
    // only. Bumping mocktime inside the loop would expire
    // `SESSION_NEW_SHARES_TIMEOUT` before real-time p2p delivers shares.
    ctx.mn_ctx.bump_mocktime(30);
