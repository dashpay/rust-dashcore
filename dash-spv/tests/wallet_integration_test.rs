...
use dashcore::{
    Address, Amount, Block, Network, OutPoint, ScriptBuf, PubkeyHash,
...
async fn test_wallet_tracks_spending() {
    ...
    let block2 = create_test_block(
        vec![coinbase_tx2, spending_tx.clone()],
        block1.block_hash(),
    );
...
async fn test_wallet_handles_reorg() {
    ...
    let outpoint_a = OutPoint { txid: tx_a.txid(), vout: 0 };
    ...
    let outpoint_b = OutPoint { txid: tx_b.txid(), vout: 0 };
    ...
    let outpoint_c = OutPoint { txid: tx_c.txid(), vout: 0 };
...
async fn test_wallet_comprehensive_scenario() {
    ...
    let alice_utxo1 = OutPoint { txid: tx1.txid(), vout: 0 };
    ...
    assert_eq!(wallet.get_balance_for_address(&alice_address).await.unwrap().total(), Amount::from_sat(alice_initial));
    ...
    let bob_utxo1 = OutPoint { txid: tx2.txid(), vout: 0 };
    ...
    assert_eq!(wallet.get_balance_for_address(&alice_address).await.unwrap().total(), Amount::from_sat(alice_initial));
    assert_eq!(wallet.get_balance_for_address(&bob_address).await.unwrap().total(), Amount::from_sat(bob_initial));
    ...
    let alice_utxo2 = OutPoint { txid: spend_tx.txid(), vout: 1 }; // Change output
    ...
    let alice_utxo3 = OutPoint { txid: transfer_tx.txid(), vout: 0 }; // From Bob
    let bob_utxo2 = OutPoint { txid: transfer_tx.txid(), vout: 1 }; // Bob's change
    ...
    assert_eq!(wallet.get_balance_for_address(&alice_address).await.unwrap().total(), Amount::from_sat(alice_final));
    assert_eq!(wallet.get_balance_for_address(&bob_address).await.unwrap().total(), Amount::from_sat(bob_final));
