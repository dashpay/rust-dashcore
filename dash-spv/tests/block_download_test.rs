...
impl NetworkManager for MockNetworkManager {
    ...
    async fn send_message(&mut self, message: NetworkMessage) -> dash_spv::error::NetworkResult<()> {
...
async fn test_process_multiple_filter_matches() {
    ...
    let result = filter_sync.process_filter_matches_and_download(filter_matches, &mut network).await;
    assert!(result.is_ok());
    
    // Should have sent 3 GetData messages
    let sent_messages = network.get_sent_messages().await;
