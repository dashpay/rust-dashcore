...
async fn test_cfheader_gap_detection_no_gap() {
    ...
    // Check gap detection
    let (has_gap, block_height, filter_height, gap_size) = filter_sync
        .check_cfheader_gap(&storage)
        .await
        .unwrap();
...
async fn test_cfheader_restart_cooldown() {
    ...
    impl NetworkManager for MockNetworkManager {
        ...
        async fn connect(&mut self) -> NetworkResult<()> { Ok(()) }
        
        async fn disconnect(&mut self) -> NetworkResult<()> { Ok(()) }
        ...
        fn peer_info(&self) -> Vec<dash_spv::types::PeerInfo> { Vec::new() }
        
        async fn send_ping(&mut self) -> NetworkResult<u64> { Ok(0) }
        
        async fn handle_ping(&mut self, _nonce: u64) -> NetworkResult<()> { Ok(()) }
        
        fn handle_pong(&mut self, _nonce: u64) -> NetworkResult<()> { Ok(()) }
    ...
    assert!(should_fail_or_be_disabled, "Third restart attempt should fail or be disabled after cooldown");
}