...
use dash_spv::{
    ...
    network::{TcpNetworkManager, NetworkManager},
...
async fn test_real_node_connectivity() {
    ...
    let peer_addr: SocketAddr = DASH_NODE_ADDR.parse()
...
async fn test_real_header_sync_genesis_to_1000() {
    ...
    // Check initial state
    let initial_progress = client.sync_progress().await
        .expect("Failed to get initial sync progress");
    
    info!("Initial sync state: height={}, synced={}", 
    ...
    match sync_result {
        Ok(Ok(progress)) => {
            ...
            assert!(progress.header_height >= 1000, 
                   "Should have synced at least 1000 headers, got: {}", progress.header_height);
            
            // Verify sync progress
            assert!(progress.header_height > initial_progress.header_height,
...
async fn test_real_header_sync_up_to_10k() {
        ...
        async {
            loop {
                ...
                if current_time.duration_since(last_report_time) >= Duration::from_secs(30) {
                    ...
                    info!("Sync progress: {} headers ({:.1} headers/sec)", 
    ...
    match sync_result {
        Ok(Ok(final_progress)) => {
            ...
            assert!(headers_synced >= 1000, 
                   "Should have synced at least 1000 headers, got: {}", headers_synced);
            
            // Performance assertions
            assert!(avg_headers_per_sec > 10.0, 
                   "Sync rate too slow: {:.1} headers/sec", avg_headers_per_sec);
            ...
            let genesis_headers = storage.load_headers(0..10).await
                .expect("Failed to load genesis headers");
            assert_eq!(genesis_headers.len(), 10);
            
            if headers_synced > 1000 {
                let mid_headers = storage.load_headers(500..510).await
                    .expect("Failed to load mid-chain headers");
                assert_eq!(mid_headers.len(), 10);
            }
            
            if headers_synced > 100 {
                let recent_start = headers_synced.saturating_sub(10);
                let recent_headers = storage.load_headers(recent_start..(recent_start + 10)).await
...
async fn test_real_header_validation_with_node() {
    ...
    match sync_result {
        Ok(Ok(progress)) => {
            ...
            assert!(progress.header_height >= 100, 
                   "Should have validated at least 100 headers, got: {}", progress.header_height);
...
async fn test_real_header_chain_continuity() {
    ...
    if headers_synced >= 100 {
        ...
        let headers = storage.load_headers(0..test_range).await
        ...
        for i in 1..headers.len() {
            ...
            assert!(headers[i].time >= headers[i-1].time,
...
async fn test_real_node_sync_resumption() {
    ...
    let mut client1 = DashSpvClient::new(config.clone()).await
        .expect("Failed to create first client");
    
    client1.start().await.expect("Failed to start first client");
    
    let initial_sync = tokio::time::timeout(
        Duration::from_secs(60),
        client1.sync_to_tip()
    ).await.expect("Initial sync timed out").expect("Initial sync failed");
    ...
    let mut client2 = DashSpvClient::new(config).await
        .expect("Failed to create second client");
    
    client2.start().await.expect("Failed to start second client");
    
    let resume_sync = tokio::time::timeout(
        Duration::from_secs(60),
        client2.sync_to_tip()
    ).await.expect("Resume sync timed out").expect("Resume sync failed");
    ...
    assert!(phase2_height >= phase1_height,
...
async fn test_real_node_performance_benchmarks() {
    ...
    let initial_progress = client.sync_progress().await
        .expect("Failed to get initial progress");
    ...
    if headers_synced > 0 {
        ...
        // Performance assertions
        assert!(headers_per_sec > 5.0, 
               "Sync performance too slow: {:.1} headers/sec", headers_per_sec);
        assert!(connection_time < Duration::from_secs(30),
