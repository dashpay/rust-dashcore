...
async fn test_segmented_storage_basic_operations() {
    ...
    assert_eq!(
        storage.get_header(0).await.unwrap().unwrap().time,
        0
    );
    assert_eq!(
        storage.get_header(49_999).await.unwrap().unwrap().time,
        49_999
    );
    assert_eq!(
        storage.get_header(50_000).await.unwrap().unwrap().time,
        50_000
    );
    assert_eq!(
        storage.get_header(99_999).await.unwrap().unwrap().time,
        99_999
    );
...
async fn test_segmented_storage_persistence() {
    ...
    // Load data in new instance
    {
        ...
        assert_eq!(
            storage.get_header(74_999).await.unwrap().unwrap().time,
            74_999
        );
...
async fn test_reverse_index_with_segments() {
    ...
    for height in [0, 25_000, 49_999, 50_000, 50_001, 75_000, 99_999] {
        ...
        assert_eq!(
            storage.get_header_height_by_hash(&hash).await.unwrap(),
            Some(height)
        );
    }
    ...
    assert_eq!(
        storage.get_header_height_by_hash(&fake_hash).await.unwrap(),
        None
    );
...
async fn test_filter_header_segments() {
    ...
    let filter_headers: Vec<FilterHeader> = (0..75_000)
        .map(create_test_filter_header)
        .collect();
...
async fn test_concurrent_access() {
    ...
    {
        let mut storage = DiskStorageManager::new(path.clone())
            .await
            .unwrap();
        let headers: Vec<BlockHeader> = (0..100_000).map(create_test_header).collect();
        storage.store_headers(&headers).await.unwrap();
...
async fn test_segment_eviction() {
    ...
    assert_eq!(
        storage.get_header(599_999).await.unwrap().unwrap().time,
        599_999
    );
...
async fn test_background_save_timing() {
    ...
    // Verify data was saved
    {
        ...
        assert_eq!(
            storage.get_header(15_000).await.unwrap().unwrap().time,
            15_000
        );
    }
}
...
async fn test_clear_storage() {
    ...
    assert_eq!(
        storage.get_header_height_by_hash(&headers[0].block_hash()).await.unwrap(),
        None
    );
}
...
async fn test_mixed_operations() {
    ...
    assert_eq!(
        storage.load_filter(1000).await.unwrap().unwrap(),
        vec![(1000 % 256) as u8; 100]
    );
    assert_eq!(
    ...
    assert_eq!(
        storage.load_metadata("test_key").await.unwrap().unwrap(),
        b"test_value"
    );
