...
fn test_transaction_62364518_net_amount_calculation() {
    ...
    *balance_changes.entry(watched_address.clone()).or_insert(0) += output_to_watched;
    ...
    println!("Input 1 (XjbaGWaGnvEtuQAUoBgDxJWe8ZNv45upG2): {} sat ({} BTC)", 
             input1_value, Amount::from_sat(input1_value as u64));
    println!("Input 2 (XjbaGWaGnvEtuQAUoBgDxJWe8ZNv45upG2): {} sat ({} BTC)", 
             input2_value, Amount::from_sat(input2_value as u64));
    println!("Input 3 (XjbaGWaGnvEtuQAUoBgDxJWe8ZNv45upG2): {} sat ({} BTC)", 
             input3_value, Amount::from_sat(input3_value as u64));
    println!("Total inputs from watched address: {} sat ({} BTC)", 
             total_inputs, Amount::from_sat(total_inputs as u64));
    println!();
    println!("Output to other address: {} sat ({} BTC)", 
             output_to_other, Amount::from_sat(output_to_other as u64));
    println!("Output back to watched address: {} sat ({} BTC)", 
             output_to_watched, Amount::from_sat(output_to_watched as u64));
    println!();
    println!("Expected net change: {} sat ({} BTC)", 
             expected_net_change, Amount::from_sat(expected_net_change.abs() as u64));
    println!("Actual net change: {} sat ({} BTC)", 
             actual_net_change, Amount::from_sat(actual_net_change.abs() as u64));
    
    // The key assertion: net change should be negative (fee + amount sent to other address)
    assert_eq!(*actual_net_change, expected_net_change, 
    ...
    println!("Transaction fee: {} sat ({} BTC)", 
             transaction_fee, Amount::from_sat(transaction_fee as u64));
...
fn test_suspected_bug_only_first_input() {
    ...
    *balance_changes.entry(watched_address.clone()).or_insert(0) += output_to_watched;
    ...
    println!("Only first input processed: {} sat ({} BTC)", 
             input1_value, Amount::from_sat(input1_value as u64));
    println!("Output to watched address: {} sat ({} BTC)", 
             output_to_watched, Amount::from_sat(output_to_watched as u64));
    println!("Buggy net change: {} sat ({} BTC)", 
             buggy_net_change, Amount::from_sat(*buggy_net_change as u64));
