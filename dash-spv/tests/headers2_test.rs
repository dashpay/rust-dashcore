...
fn test_getheaders2_message_encoding() {
    ...
    let genesis_hash = BlockHash::from_byte_array([
        0x2c, 0xbc, 0xf8, 0x3b, 0x62, 0x91, 0x3d, 0x56, 0xf6, 0x05, 0xc0, 0xe5, 0x81, 0xa4, 0x88,
        0x72, 0x83, 0x94, 0x28, 0xc9, 0x2e, 0x5e, 0xb7, 0x6c, 0xd7, 0xad, 0x94, 0xbc, 0xaf, 0x0b,
        0x00, 0x00,
    ]);
    ...
    println!(
        "Raw GetHeaders2 first 50 bytes: {:02x?}",
        &raw_serialized[..50.min(raw_serialized.len())]
    );
...
fn test_getheaders2_vs_getheaders_encoding() {
    ...
    let msg_data = GetHeadersMessage::new(vec![genesis_hash], BlockHash::all_zeros());
...
fn test_empty_locator_getheaders2() {
    ...
    let msg_data = GetHeadersMessage::new(vec![], BlockHash::all_zeros());
