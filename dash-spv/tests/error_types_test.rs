...
fn test_sync_error_variants_and_categories() {
    let test_cases = vec![
        ...
        (SyncError::InvalidState("Unexpected phase transition".to_string()), "state", "Invalid sync state: Unexpected phase transition"),
        (SyncError::MissingDependency("Previous block not found".to_string()), "dependency", "Missing dependency: Previous block not found"),
        (SyncError::Timeout("Peer response timeout".to_string()), "timeout", "Timeout error: Peer response timeout"),
        (SyncError::Network("Connection lost".to_string()), "network", "Network error: Connection lost"),
        (SyncError::Validation("Invalid block header".to_string()), "validation", "Validation error: Invalid block header"),
        (SyncError::Storage("Database locked".to_string()), "storage", "Storage error: Database locked"),
        (SyncError::Headers2DecompressionFailed("Invalid zstd stream".to_string()), "headers2", "Headers2 decompression failed: Invalid zstd stream"),
    ];
...
fn test_wallet_error_variants() {
    ...
    let errors = vec![
        ...
        (
            WalletError::InvalidScriptPubkey,
        ...
        (
            WalletError::InvalidOutput(3),
            "Invalid transaction output at index 3"
        ),
        (
...
fn test_parse_error_variants() {
    let errors = vec![
        (
            ParseError::InvalidAddress("xyz123".to_string()),
            "Invalid network address: xyz123"
        ),
        (
            ParseError::InvalidNetwork("mainnet2".to_string()),
            "Invalid network name: mainnet2"
        ),
        (
...
fn test_error_display_formatting() {
    ...
    let errors: Vec<Box<dyn std::error::Error>> = vec![
        Box::new(NetworkError::ConnectionFailed("peer1.example.com:9999 - Connection timed out after 30s".to_string())),
        Box::new(StorageError::WriteFailed("Cannot write to /var/lib/dash-spv/headers.dat: No space left on device (28)".to_string())),
        Box::new(ValidationError::InvalidHeaderChain("Block 523412: Previous block hash mismatch. Expected: 0x1234..., Got: 0x5678...".to_string())),
        Box::new(SyncError::Timeout("No response from peer after 60 seconds during header download".to_string())),
        Box::new(WalletError::TransactionValidation("Transaction abc123... has invalid signature in input 0".to_string())),
    ];
