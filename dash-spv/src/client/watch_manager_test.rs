...
#[cfg(test)]
mod tests {
    use crate::client::watch_manager::{WatchManager, WatchItemUpdateSender};
    ...
    async fn test_remove_watch_item() {
        ...
        // Remove the item
        let result = WatchManager::remove_watch_item(
            &watch_items,
            &wallet,
            &updater,
            &item,
    ...
    async fn test_watch_item_update_notification() {
        ...
        let mut storage = Box::new(MemoryStorageManager::new().await.unwrap()) as Box<dyn StorageManager>;
    ...
    async fn test_watch_item_with_label() {
        ...
        if let WatchItem::Address { label, .. } = stored_item {
    ...
    async fn test_concurrent_add_operations() {
        ...
        let items: Vec<WatchItem> = (0..5)
            .map(|i| WatchItem::Script(ScriptBuf::from(vec![0x00, i as u8])))
            .collect();
