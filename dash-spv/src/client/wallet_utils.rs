...
use crate::wallet::{Wallet, Balance};
...
impl WalletUtils {
    ...
    pub async fn safe_remove_utxo(&self, outpoint: &dashcore::OutPoint) -> Result<Option<crate::wallet::Utxo>> {
        let wallet = self.wallet.read().await;
    ...
    pub async fn sync_watch_items_with_wallet(
        ...
        for item in watch_items.iter() {
            if let crate::types::WatchItem::Address { address, .. } = item {
                let wallet = self.wallet.read().await;
