...
mod tests {
    ...
    async fn test_reorganization_no_borrow_conflict() {
        ...
        let result = reorg_manager
            .reorganize(&mut chain_state, &mut wallet_state, &fork, &mut storage)
            .await;
    ...
    async fn test_find_common_ancestor_in_main_chain() {
        ...
        let reorg_data =
            reorg_manager.collect_reorg_data(&chain_state, &fork, &storage).await.unwrap();
    ...
    async fn test_deep_reorganization() {
        ...
        for i in 100..108 {
            // 8 blocks, making fork 13 blocks total (5 + 8)
            let next = create_test_header(&current, i);
