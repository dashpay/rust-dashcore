...
use dashcore::{Address, OutPoint, Amount};
use tokio::sync::RwLock;

use crate::error::{SpvError, StorageError};
use crate::storage::StorageManager;
pub use utxo::Utxo;
pub use transaction_processor::{TransactionProcessor, TransactionResult, BlockResult, AddressStats};
...
impl Wallet {
    ...
    pub async fn load_from_storage(&self) -> Result<(), SpvError> {
        ...
        if let Some(data) = storage.load_metadata("watched_addresses").await? {
            let address_strings: Vec<String> = bincode::deserialize(&data)
                .map_err(|e| SpvError::Storage(StorageError::Serialization(format!("Failed to deserialize watched addresses: {}", e))))?;
            
            let mut addresses = HashSet::new();
            for addr_str in address_strings {
                let address = addr_str.parse::<dashcore::Address<dashcore::address::NetworkUnchecked>>()
                    .map_err(|e| SpvError::Storage(StorageError::Serialization(format!("Invalid address: {}", e))))?
                    .assume_checked();
    ...
    async fn calculate_balance(&self, address_filter: Option<&Address>) -> Result<Balance, SpvError> {
        let utxos = self.utxo_set.read().await;
    ...
    async fn save_watched_addresses(&self, addresses: &HashSet<Address>) -> Result<(), SpvError> {
        ...
        let data = bincode::serialize(&address_strings)
            .map_err(|e| SpvError::Storage(StorageError::Serialization(format!("Failed to serialize watched addresses: {}", e))))?;
...
mod tests {
    ...
    fn create_test_address() -> Address {
        ...
        use dashcore::{Address, ScriptBuf, PubkeyHash};
    ...
    async fn test_calculate_balance_multiple_utxos() {
        ...
        let utxo3 = crate::wallet::Utxo::new(
            OutPoint {
                txid: Txid::from_str("3333333333333333333333333333333333333333333333333333333333333333").unwrap(),
                vout: 0,
            },
            TxOut {
                value: 500000,
