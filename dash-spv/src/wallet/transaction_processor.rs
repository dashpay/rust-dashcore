...
impl TransactionProcessor {
    ...
    async fn process_transaction(
        ...
        if !is_coinbase {
            for input in &transaction.input {
                ...
                if let Some(spent_utxo) = wallet.remove_utxo(&outpoint).await? {
                    ...
                    tracing::debug!("💸 UTXO spent: {} (value: {})", outpoint, spent_utxo.value());
                }
            }
        }
        ...
        for (vout, output) in transaction.output.iter().enumerate() {
            ...
            if let Some(watched_address) =
                watched_addresses.iter().find(|addr| addr.script_pubkey() == output.script_pubkey)
            {
...
mod tests {
    ...
    async fn test_process_empty_block() {
        ...
        let result = processor
            ...
            .await
            .expect("Should process block at height 100 successfully");
    ...
    async fn test_process_block_with_regular_transaction_to_watched_address() {
        ...
        let result = processor
            .process_block(&block, 200, &wallet, &mut storage)
            .await
            .expect("Should process block at height 200 successfully");
    ...
    async fn test_process_block_with_spending_transaction() {
        ...
        let result = processor
            .process_block(&block, 300, &wallet, &mut storage)
            .await
            .expect("Should process block at height 300 successfully");
    ...
    async fn test_process_block_with_irrelevant_transactions() {
        ...
        let result = processor
            .process_block(&block, 400, &wallet, &mut storage)
            .await
            .expect("Should process block at height 400 successfully");
