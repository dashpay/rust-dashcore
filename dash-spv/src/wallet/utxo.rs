        ...
        D: Deserializer<'de>,
    {
        ...
        impl<'de> Visitor<'de> for UtxoVisitor {
                ...
                M: MapAccess<'de>,
            {
                ...
                let outpoint =
                    outpoint.ok_or_else(|| serde::de::Error::missing_field("outpoint"))?;
                let txout = txout.ok_or_else(|| serde::de::Error::missing_field("txout"))?;
                let address_str =
                    address_str.ok_or_else(|| serde::de::Error::missing_field("address"))?;
                let height = height.ok_or_else(|| serde::de::Error::missing_field("height"))?;
                let is_coinbase =
                    is_coinbase.ok_or_else(|| serde::de::Error::missing_field("is_coinbase"))?;
                let is_confirmed =
                    is_confirmed.ok_or_else(|| serde::de::Error::missing_field("is_confirmed"))?;
                let is_instantlocked = is_instantlocked
                    .ok_or_else(|| serde::de::Error::missing_field("is_instantlocked"))?;
