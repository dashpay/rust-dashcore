...
use dashcore::{BlockHash, block::Header as BlockHeader, Address, OutPoint, Txid};
use std::ops::Range;
...
pub trait StorageBackend: Send + Sync + 'static {
    ...
    async fn get_utxos_for_address(&self, address: &Address) -> StorageResult<Vec<(OutPoint, Utxo)>>;
    ...
    async fn add_mempool_transaction(&mut self, txid: &Txid, tx: &UnconfirmedTransaction) -> StorageResult<()>;
...
impl StorageService {
    ...
    pub async fn run(mut self) {
        ...
        while let Some(command) = self.command_rx.recv().await {
            // Don't log GetFilterTipHeight commands as they're too frequent and can cause logging bottlenecks
        match &command {
    ...
    async fn process_command(&mut self, command: StorageCommand) {
        match command {
            // Header operations
            StorageCommand::StoreHeader { header, height, response } => {
                tracing::trace!("StorageService: processing StoreHeader for height {}", height);
                tracing::debug!("[HANG DEBUG] StorageService: Starting to process StoreHeader for height {}", height);
                
                // Check if sender is closed before processing
                if response.is_closed() {
                    tracing::error!("[HANG DEBUG] Response sender is already closed at start of processing!");
                }
                
                let start = std::time::Instant::now();
                
                tracing::debug!("[HANG DEBUG] StorageService: About to call backend.store_header for height {}", height);
                let result = self.backend.store_header(&header, height).await;
                tracing::debug!("[HANG DEBUG] StorageService: backend.store_header returned for height {}: {:?}", height, result.is_ok());
                ...
                tracing::debug!("[HANG DEBUG] StorageService: About to send response for height {}", height);
                let send_result = response.send(result);
                tracing::debug!("[HANG DEBUG] StorageService: response.send completed for height {}: {:?}", height, send_result.is_ok());
            }
            StorageCommand::GetHeader { height, response } => {
                let result = self.backend.get_header(height).await;
                let _ = response.send(result);
            }
            StorageCommand::GetHeaderByHash { hash, response } => {
                let result = self.backend.get_header_by_hash(&hash).await;
                let _ = response.send(result);
            }
            StorageCommand::GetHeaderHeight { hash, response } => {
                let result = self.backend.get_header_height(&hash).await;
                let _ = response.send(result);
            }
            StorageCommand::GetTipHeight { response } => {
                let result = self.backend.get_tip_height().await;
                let _ = response.send(result);
            }
            StorageCommand::LoadHeaders { range, response } => {
                let result = self.backend.load_headers(range).await;
                let _ = response.send(result);
            }
            
            // Filter operations
            StorageCommand::StoreFilterHeader { header, height, response } => {
                let result = self.backend.store_filter_header(&header, height).await;
                let _ = response.send(result);
            }
            StorageCommand::GetFilterHeader { height, response } => {
                let result = self.backend.get_filter_header(height).await;
                let _ = response.send(result);
            }
            StorageCommand::GetFilterTipHeight { response } => {
                // Process without logging to avoid flooding logs
                let result = self.backend.get_filter_tip_height().await;
                let _ = response.send(result);
            }
            StorageCommand::StoreFilter { filter, height, response } => {
                let result = self.backend.store_filter(&filter, height).await;
                let _ = response.send(result);
            }
            StorageCommand::GetFilter { height, response } => {
                let result = self.backend.get_filter(height).await;
                let _ = response.send(result);
            }
            
            // State operations
            StorageCommand::SaveMasternodeState { state, response } => {
                let result = self.backend.save_masternode_state(&state).await;
                let _ = response.send(result);
            }
            StorageCommand::LoadMasternodeState { response } => {
                let result = self.backend.load_masternode_state().await;
                let _ = response.send(result);
            }
            StorageCommand::StoreChainState { state, response } => {
                let result = self.backend.store_chain_state(&state).await;
                let _ = response.send(result);
            }
            StorageCommand::LoadChainState { response } => {
                let result = self.backend.load_chain_state().await;
                let _ = response.send(result);
            }
            
            // UTXO operations
            StorageCommand::StoreUtxo { outpoint, utxo, response } => {
                let result = self.backend.store_utxo(&outpoint, &utxo).await;
                let _ = response.send(result);
            }
            StorageCommand::RemoveUtxo { outpoint, response } => {
                let result = self.backend.remove_utxo(&outpoint).await;
                let _ = response.send(result);
            }
            StorageCommand::GetUtxo { outpoint, response } => {
                let result = self.backend.get_utxo(&outpoint).await;
                let _ = response.send(result);
            }
            StorageCommand::GetUtxosForAddress { address, response } => {
                let result = self.backend.get_utxos_for_address(&address).await;
                let _ = response.send(result);
            }
            StorageCommand::GetAllUtxos { response } => {
                let result = self.backend.get_all_utxos().await;
                let _ = response.send(result);
            }
            
            // Mempool operations
            StorageCommand::SaveMempoolState { state, response } => {
                let result = self.backend.save_mempool_state(&state).await;
                let _ = response.send(result);
            }
            StorageCommand::LoadMempoolState { response } => {
                let result = self.backend.load_mempool_state().await;
                let _ = response.send(result);
            }
            StorageCommand::AddMempoolTransaction { txid, tx, response } => {
                let result = self.backend.add_mempool_transaction(&txid, &tx).await;
                let _ = response.send(result);
            }
            StorageCommand::RemoveMempoolTransaction { txid, response } => {
                let result = self.backend.remove_mempool_transaction(&txid).await;
                let _ = response.send(result);
            }
            StorageCommand::GetMempoolTransaction { txid, response } => {
...
impl StorageClient {
    ...
    pub async fn store_header(&self, header: &BlockHeader, height: u32) -> StorageResult<()> {
        ...
        if tx.is_closed() {
            tracing::error!("[HANG DEBUG] Receiver already closed immediately after channel creation!");
        }
        
        tracing::trace!("StorageClient: sending StoreHeader command for height {}", height);
        let send_start = std::time::Instant::now();
        
        tracing::debug!("[HANG DEBUG] StorageClient: About to send command for height {}", height);
        // Check channel capacity
        if self.command_tx.capacity() == 0 {
            tracing::warn!("[HANG DEBUG] Command channel is at full capacity!");
        }
        
        let send_result = self.command_tx.send(StorageCommand::StoreHeader {
            header: *header,
            height,
            response: tx,
        }).await;
        ...
        match send_result {
            Ok(_) => {
                tracing::debug!("[HANG DEBUG] StorageClient: Command sent successfully for height {}", height);
                // Give the service a chance to process the command
                tokio::task::yield_now().await;
            }
            Err(e) => {
                tracing::error!("[HANG DEBUG] StorageClient: Failed to send command for height {}: {:?}", height, e);
        ...
        tracing::debug!("[HANG DEBUG] StorageClient: About to wait for response on rx for height {}", height);
        ...
        impl Drop for DropGuard {
            fn drop(&mut self) {
                tracing::error!("[HANG DEBUG] DropGuard dropped for height {}!", self.height);
            }
        }
        
        let _guard = DropGuard { height };
        
        tracing::debug!("[HANG DEBUG] StorageClient: Starting rx.await for height {}", height);
        let rx_result = rx.await;
        tracing::debug!("[HANG DEBUG] StorageClient: rx.await returned for height {}: {:?}", height, rx_result.is_ok());
        
        let result = rx_result.map_err(|e| {
            tracing::error!("[HANG DEBUG] StorageClient: Failed to receive response for height {}: {:?}", height, e);
        ...
        tracing::debug!("[HANG DEBUG] StorageClient: store_header completed for height {}: {:?}", height, result.is_ok());
        result
    }
    
    pub async fn get_header(&self, height: u32) -> StorageResult<Option<BlockHeader>> {
        let (tx, rx) = oneshot::channel();
        self.command_tx.send(StorageCommand::GetHeader {
            height,
            response: tx,
        }).await
    ...
    pub async fn get_header_by_hash(&self, hash: &BlockHash) -> StorageResult<Option<BlockHeader>> {
        let (tx, rx) = oneshot::channel();
        self.command_tx.send(StorageCommand::GetHeaderByHash {
            hash: *hash,
            response: tx,
        }).await
            .map_err(|_| StorageError::ServiceUnavailable)?;
    ...
    pub async fn get_header_height(&self, hash: &BlockHash) -> StorageResult<Option<u32>> {
        let (tx, rx) = oneshot::channel();
        self.command_tx.send(StorageCommand::GetHeaderHeight {
            hash: *hash,
            response: tx,
        }).await
            .map_err(|_| StorageError::ServiceUnavailable)?;
    ...
    pub async fn get_tip_height(&self) -> StorageResult<Option<u32>> {
        let (tx, rx) = oneshot::channel();
        self.command_tx.send(StorageCommand::GetTipHeight {
            response: tx,
        }).await
            .map_err(|_| StorageError::ServiceUnavailable)?;
        rx.await.map_err(|_| StorageError::ServiceUnavailable)?
    ...
    pub async fn load_headers(&self, range: Range<u32>) -> StorageResult<Vec<BlockHeader>> {
        let (tx, rx) = oneshot::channel();
        self.command_tx.send(StorageCommand::LoadHeaders {
            range,
            response: tx,
        }).await
            .map_err(|_| StorageError::ServiceUnavailable)?;
    ...
    // Filter operations
    pub async fn store_filter_header(&self, header: &FilterHeader, height: u32) -> StorageResult<()> {
        let (tx, rx) = oneshot::channel();
        self.command_tx.send(StorageCommand::StoreFilterHeader {
            header: *header,
            height,
            response: tx,
        }).await
    ...
    pub async fn get_filter_header(&self, height: u32) -> StorageResult<Option<FilterHeader>> {
        let (tx, rx) = oneshot::channel();
        self.command_tx.send(StorageCommand::GetFilterHeader {
            height,
            response: tx,
        }).await
            .map_err(|_| StorageError::ServiceUnavailable)?;
    ...
    pub async fn get_filter_tip_height(&self) -> StorageResult<Option<u32>> {
        let (tx, rx) = oneshot::channel();
        self.command_tx.send(StorageCommand::GetFilterTipHeight {
            response: tx,
        }).await
            .map_err(|_| StorageError::ServiceUnavailable)?;
        rx.await.map_err(|_| StorageError::ServiceUnavailable)?
    ...
    pub async fn store_filter(&self, filter: &[u8], height: u32) -> StorageResult<()> {
        let (tx, rx) = oneshot::channel();
        self.command_tx.send(StorageCommand::StoreFilter {
    ...
    pub async fn get_filter(&self, height: u32) -> StorageResult<Option<Vec<u8>>> {
        let (tx, rx) = oneshot::channel();
        self.command_tx.send(StorageCommand::GetFilter {
            height,
            response: tx,
        }).await
            .map_err(|_| StorageError::ServiceUnavailable)?;
    ...
    pub async fn save_masternode_state(&self, state: &MasternodeState) -> StorageResult<()> {
        let (tx, rx) = oneshot::channel();
        self.command_tx.send(StorageCommand::SaveMasternodeState {
            state: state.clone(),
            response: tx,
        }).await
            .map_err(|_| StorageError::ServiceUnavailable)?;
    ...
    pub async fn load_masternode_state(&self) -> StorageResult<Option<MasternodeState>> {
        let (tx, rx) = oneshot::channel();
        self.command_tx.send(StorageCommand::LoadMasternodeState {
            response: tx,
        }).await
            .map_err(|_| StorageError::ServiceUnavailable)?;
        rx.await.map_err(|_| StorageError::ServiceUnavailable)?
    ...
    pub async fn store_chain_state(&self, state: &ChainState) -> StorageResult<()> {
        let (tx, rx) = oneshot::channel();
        self.command_tx.send(StorageCommand::StoreChainState {
            state: state.clone(),
            response: tx,
        }).await
            .map_err(|_| StorageError::ServiceUnavailable)?;
    ...
    pub async fn load_chain_state(&self) -> StorageResult<Option<ChainState>> {
        let (tx, rx) = oneshot::channel();
        self.command_tx.send(StorageCommand::LoadChainState {
            response: tx,
        }).await
            .map_err(|_| StorageError::ServiceUnavailable)?;
        rx.await.map_err(|_| StorageError::ServiceUnavailable)?
    ...
    pub async fn store_utxo(&self, outpoint: &OutPoint, utxo: &Utxo) -> StorageResult<()> {
        let (tx, rx) = oneshot::channel();
        self.command_tx.send(StorageCommand::StoreUtxo {
            outpoint: *outpoint,
            utxo: utxo.clone(),
            response: tx,
        }).await
    ...
    pub async fn remove_utxo(&self, outpoint: &OutPoint) -> StorageResult<()> {
        let (tx, rx) = oneshot::channel();
        self.command_tx.send(StorageCommand::RemoveUtxo {
            outpoint: *outpoint,
            response: tx,
        }).await
            .map_err(|_| StorageError::ServiceUnavailable)?;
    ...
    pub async fn get_utxo(&self, outpoint: &OutPoint) -> StorageResult<Option<Utxo>> {
        let (tx, rx) = oneshot::channel();
        self.command_tx.send(StorageCommand::GetUtxo {
            outpoint: *outpoint,
            response: tx,
        }).await
            .map_err(|_| StorageError::ServiceUnavailable)?;
    ...
    pub async fn get_utxos_for_address(&self, address: &Address) -> StorageResult<Vec<(OutPoint, Utxo)>> {
        let (tx, rx) = oneshot::channel();
        self.command_tx.send(StorageCommand::GetUtxosForAddress {
    ...
    pub async fn get_all_utxos(&self) -> StorageResult<Vec<(OutPoint, Utxo)>> {
        let (tx, rx) = oneshot::channel();
        self.command_tx.send(StorageCommand::GetAllUtxos {
            response: tx,
        }).await
            .map_err(|_| StorageError::ServiceUnavailable)?;
        rx.await.map_err(|_| StorageError::ServiceUnavailable)?
    ...
    pub async fn save_mempool_state(&self, state: &MempoolState) -> StorageResult<()> {
        let (tx, rx) = oneshot::channel();
        self.command_tx.send(StorageCommand::SaveMempoolState {
            state: state.clone(),
            response: tx,
        }).await
            .map_err(|_| StorageError::ServiceUnavailable)?;
    ...
    pub async fn load_mempool_state(&self) -> StorageResult<Option<MempoolState>> {
        let (tx, rx) = oneshot::channel();
        self.command_tx.send(StorageCommand::LoadMempoolState {
            response: tx,
        }).await
            .map_err(|_| StorageError::ServiceUnavailable)?;
        rx.await.map_err(|_| StorageError::ServiceUnavailable)?
    ...
    pub async fn add_mempool_transaction(&self, txid: &Txid, tx: &UnconfirmedTransaction) -> StorageResult<()> {
        let (tx_send, rx) = oneshot::channel();
        self.command_tx.send(StorageCommand::AddMempoolTransaction {
            txid: *txid,
            tx: tx.clone(),
            response: tx_send,
        }).await
    ...
    pub async fn remove_mempool_transaction(&self, txid: &Txid) -> StorageResult<()> {
        let (tx, rx) = oneshot::channel();
        self.command_tx.send(StorageCommand::RemoveMempoolTransaction {
            txid: *txid,
            response: tx,
        }).await
            .map_err(|_| StorageError::ServiceUnavailable)?;
    ...
    pub async fn get_mempool_transaction(&self, txid: &Txid) -> StorageResult<Option<UnconfirmedTransaction>> {
        let (tx, rx) = oneshot::channel();
        self.command_tx.send(StorageCommand::GetMempoolTransaction {
            txid: *txid,
            response: tx,
        }).await
            .map_err(|_| StorageError::ServiceUnavailable)?;
    ...
    pub async fn clear_mempool(&self) -> StorageResult<()> {
        let (tx, rx) = oneshot::channel();
        self.command_tx.send(StorageCommand::ClearMempool {
            response: tx,
        }).await
            .map_err(|_| StorageError::ServiceUnavailable)?;
        rx.await.map_err(|_| StorageError::ServiceUnavailable)?
