...
impl HeaderValidator {
    ...
    pub fn validate_chain_full(
        ...
        for i in 0..headers.len() {
            let header = &headers[i];
            let prev_header = if i > 0 { Some(&headers[i - 1]) } else { None };
    ...
    pub fn validate_connects_to_genesis(&self, headers: &[BlockHeader]) -> ValidationResult<()> {
        ...
        let genesis_hash = self.network.known_genesis_block_hash()
            .ok_or_else(|| ValidationError::Consensus("No known genesis hash for network".to_string()))?;
