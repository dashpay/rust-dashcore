    ...
    FiltersManager<H, FH, F, W>
{
    ...
    async fn load_filters(
        ...
        let loaded_filters = self
            .filter_storage
            .read()
            .await
            .load_filters(start_height..end_height + 1)
            .await
            .map_err(|e| SyncError::Storage(format!("Failed to load filters: {}", e)))?;

        let loaded_headers = self
            .header_storage
            .read()
            .await
            .load_headers(start_height..end_height + 1)
            .await
    ...
    pub(super) async fn start_download(
        ...
        let (wallet_birth_height, wallet_synced_height) = {
            ...
            (wallet.earliest_required_height().await, wallet.synced_height().await)
        };

        // Get stored filters tip
        let stored_filters_tip = self
            .filter_storage
            .read()
            .await
            .filter_tip_height()
        ...
        while let Some(batch) = self.pending_batches.first() {
            ...
            if !batch.verified() {
                ...
                let filter_headers = self
                    ...
                    .await
                    .map_err(|e| {
                        SyncError::Storage(format!("Failed to load filter headers: {}", e))
                    })?;
                ...
                for (key, filter) in batch.filters() {
                    filter_storage.store_filter(key.height(), &filter.content).await.map_err(
                        |e| SyncError::Storage(format!("Failed to store filter: {}", e)),
                    )?;
                }
                drop(filter_storage);
    ...
    async fn try_commit_batches(&mut self) -> SyncResult<Vec<SyncEvent>> {
        ...
        loop {
            ...
            self.wallet.write().await.set_synced_height(batch.end_height()).await;
