...
pub struct InstantSendManager {
    ...
    /// Pending InstantLocks awaiting validation.
    pub(super) pending_instantlocks: Vec<InstantLock>,
}
...
impl InstantSendManager {
    ...
    pub(super) async fn process_instantlock(
        ...
        if !validated {
            self.queue_pending(instantlock.clone());
        }
    ...
    fn queue_pending(&mut self, instantlock: InstantLock) {
        // Remove oldest if at capacity
        if self.pending_instantlocks.len() >= MAX_PENDING_INSTANTLOCKS {
            self.pending_instantlocks.remove(0);
        }
        self.pending_instantlocks.push(instantlock);
    }
    ...
    pub(super) async fn validate_pending(&mut self) -> SyncResult<Vec<SyncEvent>> {
        ...
        for instantlock in pending {
            ...
            if validated {
                ...
                self.queue_pending(instantlock);
            }
        }
