...
impl<H: BlockHeaderStorage> SyncManager for ChainLockManager<H> {
    ...
    async fn handle_sync_event(
        ...
        // Validate pending ChainLocks when masternode state is updated
        if let SyncEvent::MasternodeStateUpdated {
            ..
        } = event
        {
            let events = if !self.pending_chainlocks.is_empty() {
                tracing::info!(
                    "Masternode state updated, validating {} pending ChainLocks",
            ...
            if self.pending_chainlocks.is_empty()
                && matches!(self.state(), SyncState::Syncing | SyncState::WaitForEvents)
            {
                self.set_state(SyncState::Synced);
                tracing::info!("ChainLock manager synced (no pending validations)");
            }
