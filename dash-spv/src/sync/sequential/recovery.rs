...
pub enum RecoveryStrategy {
    ...
    RestartPhase { checkpoint: PhaseCheckpoint },
    ...
    WaitForNetwork { timeout: Duration },
}
...
pub enum PreservedProgress {
    Headers { validated_up_to: u32 },
    FilterHeaders { validated_up_to: u32 },
    Filters { completed_heights: Vec<u32> },
    Blocks { downloaded_hashes: Vec<dashcore::BlockHash> },
    None,
}
...
impl RecoveryManager {
    ...
    pub fn determine_strategy(
        ...
        match (phase, error_type.as_str()) {
            ...
            (SyncPhase::DownloadingHeaders { current_height, .. }, "validation") => {
    ...
    pub async fn execute_recovery(
        ...
        tracing::info!(
            "🔧 Executing recovery strategy {:?} for phase {}",
            strategy,
            phase_name
        );
        ...
        let strategy_clone = match &strategy {
            RecoveryStrategy::Retry { delay } => RecoveryStrategy::Retry { delay: *delay },
            RecoveryStrategy::RestartPhase { checkpoint } => RecoveryStrategy::RestartPhase { checkpoint: checkpoint.clone() },
            RecoveryStrategy::SkipPhase { reason } => RecoveryStrategy::SkipPhase { reason: reason.clone() },
            ...
            RecoveryStrategy::WaitForNetwork { timeout } => RecoveryStrategy::WaitForNetwork { timeout: *timeout },
        };
        ...
        let result = match strategy {
            ...
            RecoveryStrategy::SkipPhase { reason } => {
            ...
            RecoveryStrategy::WaitForNetwork { timeout } => {
