...
impl ValidationStateManager {
    ...
    pub fn rollback_to_snapshot(&mut self, snapshot_id: u64) -> SyncResult<()> {
        let snapshot = self.snapshots
            ...
            .ok_or_else(|| SyncError::InvalidState(format!("Snapshot {} not found", snapshot_id)))?;
    ...
    pub fn rollback_to_latest(&mut self) -> SyncResult<()> {
        let snapshot = self.snapshots
            .back()
            .ok_or_else(|| SyncError::InvalidState("No snapshots available for rollback".to_string()))?;
    ...
    pub fn add_pending_validation(&mut self, height: u32, validation_type: ValidationType) {
        self.current_state.pending_validations.insert(height, PendingValidation {
            height,
            validation_type,
            retry_count: 0,
    ...
    pub fn complete_validation(&mut self, height: u32) -> Option<PendingValidation> {
        ...
        if result.is_some() {
            self.current_state.last_validated_height = self.current_state.last_validated_height.max(height);
            self.current_state.version += 1;
            self.notify_listeners();
        }
        result
    ...
    pub fn record_validation_failure(
        ...
        self.current_state.validation_failures
            .entry(height)
    ...
    pub fn update_quorum_validation(
        ...
        if let Some(state) = self.current_state.active_quorum_validations.get_mut(&key) {
            ...
            self.current_state.active_quorum_validations.insert(key, QuorumValidationState {
                quorum_type,
                quorum_hash,
                status,
                members_validated: 0,
    ...
    fn cleanup_expired_snapshots(&mut self) {
        ...
        self.snapshots.retain(|snapshot| {
            now.duration_since(snapshot.created_at) < self.snapshot_ttl
        });
    }
    ...
    pub fn get_stats(&self) -> ValidationStats {
        ValidationStats {
            ...
            total_failures: self.current_state.validation_failures.values()
                .map(|v| v.len())
                .sum(),
