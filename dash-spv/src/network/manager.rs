...
impl PeerNetworkManager {
    ...
    pub fn network_event_bus(&self) -> &EventBus<NetworkEvent> {
        &self.network_event_bus
    }
    ...
    async fn connect_to_peer(&self, addr: SocketAddr) {
        ...
        let network_event_bus = self.network_event_bus.clone();
                    ...
                    request = request_rx.recv() => {
                        match request {
                            Some(NetworkRequest::SendMessage(msg)) => {
                                ...
                                // - Headers: sticky peer (chain consistency)
                                // - CFHeaders/CFilters/Blocks: distribute across peers (with timeout/retry)
                                //
                                // Spawn each send concurrently to allow parallel requests across peers.
                                // Without this, the processor would await each send sequentially,
                                // defeating the benefit of round-robin distribution.
                                let this = this.clone();
                                tokio::spawn(async move {
                                    let result = match &msg {
                                        ...
                                        _ => {
                                            // Other messages use sticky peer
...
impl Clone for PeerNetworkManager {
    fn clone(&self) -> Self {
        Self {
            ...
            network_event_bus: self.network_event_bus.clone(),
        }
    }
}
