...
impl Peer {
    ...
    pub async fn send_message(&mut self, message: NetworkMessage) -> NetworkResult<()> {
        ...
        // Clear connection state on write-path disconnects so the reader loop observes
        // `PeerDisconnected` the same way it would for a read-path EOF.
        if result.is_err() && !transport.is_connected() {
            self.transport = None;
            self.connected_at = None;
        }
    ...
    pub async fn receive_message(&mut self) -> NetworkResult<Option<Message>> {
        // If the transport was cleared e.g. by a write-path broken pipe, treat as disconnected
        // so the reader loop handles it identically to a read-path EOF.
        let transport = self.transport.as_mut().ok_or(NetworkError::PeerDisconnected)?;
        ...
        result.map(|message| message.map(|inner| Message::new(self.address, inner)))
    }
