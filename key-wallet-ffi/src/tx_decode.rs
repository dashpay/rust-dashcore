...
//!
//! `transaction_deserialize` returns an opaque [`crate::transaction::FFITransaction`]
//! handle with no read accessors, so an FFI consumer that needs to match
//! arbitrary transactions against external addresses (e.g. the CrowdNode
//! on-chain API: "did a tx pay amount X to address Y?") needs per-output
//! `(address, amount)` reconstructed from the raw bytes. This entry point
//! decodes straight into [`Transaction`] and marshals that into the C
//! structures below — no intermediate representation.
//!
...
//! - `txid` / `prev_txid` are in consensus (internal) byte order — reverse
//!   for explorer-style display.

use std::ffi::CString;
use std::os::raw::c_char;

use crate::error::{FFIError, FFIErrorCode};
use crate::{check_ptr, unwrap_or_return};
use dash_network::ffi::FFINetwork;
use dashcore::blockdata::script::Instruction;
use dashcore::consensus::deserialize;
use dashcore::hashes::Hash;
use dashcore::{Address, Network, PublicKey, ScriptBuf, Transaction};
...
/// A consensus-decoded transaction. Allocated by `transaction_decode`;
/// release with `decoded_transaction_free`.
#[repr(C)]
...
/// Boxed-slice allocator for arrays owned by `DecodedTransactionFFI`.
...
///   released with `decoded_transaction_free`.
/// - `error` must be a valid pointer to an FFIError.
#[no_mangle]
pub unsafe extern "C" fn transaction_decode(
    tx_bytes: *const u8,
    tx_bytes_len: usize,
    network: FFINetwork,
    ...
    check_ptr!(out_decoded, error);
    *out_decoded = std::ptr::null_mut();
    check_ptr!(tx_bytes, error);
    if tx_bytes_len == 0 {
        (*error).set(FFIErrorCode::InvalidInput, "tx_bytes_len is zero");
        return false;
    }

    let network: Network = network.into();
    let slice = std::slice::from_raw_parts(tx_bytes, tx_bytes_len);
    let tx: Transaction = unwrap_or_return!(deserialize(slice), error);
...
/// Release a `DecodedTransactionFFI` returned by `transaction_decode`.
/// Safe to call with null.
///
/// # Safety
/// `decoded` must be null or a pointer previously returned by
/// `transaction_decode` that has not been freed yet.
#[no_mangle]
pub unsafe extern "C" fn decoded_transaction_free(decoded: *mut DecodedTransactionFFI) {
    if decoded.is_null() {
        return;
    }
    ...
    if !decoded.outputs.is_null() {
        let outputs =
            Vec::from_raw_parts(decoded.outputs, decoded.outputs_count, decoded.outputs_count);
...
mod tests {
    ...
    //! serialize → `transaction_decode` → plain-Rust copy → assert.
    //! `decode_ok` owns the pointer walking and frees exactly once.
    ...
    fn decode_ok(tx: &Transaction, network: FFINetwork) -> Decoded {
        ...
        unsafe {
            ...
            let ok = transaction_decode(bytes.as_ptr(), bytes.len(), network, &mut out, &mut error);
            assert!(ok);
    ...
    fn decodes_outputs_with_addresses_and_values() {
        ...
        assert!(addr.to_string().starts_with('y'), "testnet P2PKH starts with 'y'");
    }
    ...
    fn op_return_output_has_no_address() {
        ...
        assert!(!decoded.outputs[1].2.is_empty(), "script bytes still present");
    }
    ...
    fn rejects_null_and_empty_input() {
        unsafe {
            ...
            let mut out: *mut DecodedTransactionFFI = std::ptr::NonNull::dangling().as_ptr();
            let ok =
                transaction_decode(std::ptr::null(), 4, FFINetwork::Testnet, &mut out, &mut error);
            assert!(!ok);
            assert_eq!(error.code, FFIErrorCode::InvalidInput);
            assert!(out.is_null(), "out param is nulled on failure");
            ...
            let ok =
                transaction_decode(bytes.as_ptr(), 0, FFINetwork::Testnet, &mut out, &mut error);
            assert!(!ok);
            assert_eq!(error.code, FFIErrorCode::InvalidInput);
            assert!(out.is_null(), "out param is nulled on failure");
        }
    ...
    fn free_null_is_safe() {
        unsafe { decoded_transaction_free(std::ptr::null_mut()) }
    }
}
