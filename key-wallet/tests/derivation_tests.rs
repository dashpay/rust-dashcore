...
fn test_dip17_platform_payment_vector1_mainnet() {
    ...
    let addr = Address::new(
        ...
        Payload::PlatformPubkeyHash(pubkey_hash),
    );
    assert_eq!(
        addr.to_string(),
...
fn test_dip17_platform_payment_vector2() {
    ...
    let addr_mainnet = Address::new(
        dashcore::Network::Dash,
        Payload::PlatformPubkeyHash(pubkey_hash_mainnet),
    );
    assert_eq!(
    ...
    let addr_testnet = Address::new(
        dashcore::Network::Testnet,
        Payload::PlatformPubkeyHash(pubkey_hash_testnet),
    );
    assert_eq!(
