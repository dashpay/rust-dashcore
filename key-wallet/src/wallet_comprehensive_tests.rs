...
mod tests {
    ...
    fn test_multiple_account_management() {
        ...
        wallet.create_account(1, AccountType::Standard, Network::Testnet).unwrap();
        wallet.create_account(2, AccountType::CoinJoin, Network::Testnet).unwrap();
    ...
    fn test_coinjoin_account_creation() {
        ...
        let wallet = Wallet::new(config, Network::Testnet).unwrap();
    ...
    fn test_mnemonic_multiple_languages() {
        ...
        for lang in languages {
            let mnemonic = Mnemonic::generate(12, lang).unwrap();
            // Language verification is done implicitly by successful generation

            // Verify we can create a wallet
            let config = WalletConfig::default();

            let wallet = Wallet::from_mnemonic(mnemonic, config, Network::Testnet);
            assert!(wallet.is_ok());
        }
    }
    ...
    fn test_large_address_pool_generation() {
        ...
        assert_eq!(pool.stats().highest_generated, Some(999));
    ...
    fn test_address_pool_pruning() {
        ...
        assert!(pruned > 0);
        assert!(pool.stats().highest_generated.unwrap_or(0) <= 50);
    }
    ...
    fn test_watch_only_wallet_complete_workflow() {
        ...
        let wallet = Wallet::from_mnemonic(mnemonic, config.clone(), Network::Testnet).unwrap();
    ...
    fn test_identity_derivation_paths() {
        ...
        assert_ne!(reg_key.private_key.secret_bytes(), topup_key.private_key.secret_bytes());
    }
    ...
    fn test_wallet_bip38_export_import() {
        ...
        assert!(base58.starts_with("6")); // BIP38 encrypted keys start with 6 (6P for Bitcoin, different for Dash)
    ...
    fn test_bip38_with_different_networks() {
        ...
        // Test mainnet
        let mainnet_encrypted =
            encrypt_private_key(&private_key, password, true, Network::Dash).unwrap();

        // Test testnet
        let testnet_encrypted =
            encrypt_private_key(&private_key, password, true, Network::Testnet).unwrap();
    ...
    fn test_wallet_full_lifecycle() {
        ...
        let mut wallet = Wallet::new(config, Network::Testnet).unwrap();
        let original_mnemonic = wallet.test_get_mnemonic().cloned();
        ...
        let restored_config = WalletConfig::default();

        let restored = Wallet::restore(backup, restored_config, Network::Testnet).unwrap();

        // 6. Verify restoration
        assert_eq!(restored.accounts.len(), 2);
        if let (Some(orig), Some(rest)) = (original_mnemonic, restored.test_get_mnemonic()) {
    ...
    fn test_wallet_concurrent_operations() {
        ...
        let wallet = Arc::new(Mutex::new(Wallet::new(config, Network::Testnet).unwrap()));
        let mut handles = vec![];
