...
impl Bip38EncryptedKey {
    ...
    fn decrypt_non_ec_multiply(&self, password: &str) -> Result<SecretKey> {
        ...
        scrypt::scrypt(
            password.as_bytes(),
            address_hash,
            &scrypt::Params::new(14, SCRYPT_R, SCRYPT_P).unwrap(),
            &mut derived_key,
    ...
    fn decrypt_ec_multiply(&self, password: &str) -> Result<SecretKey> {
        ...
        let pass_point_bytes = if self.compressed {
            pass_point.serialize()
        } else {
            pass_point.serialize_uncompressed()
        };
        
        scrypt::scrypt(
            &pass_point_bytes,
            address_hash,
            &scrypt::Params::new(10, 1, 1).unwrap(),
            &mut derived_key,
        ).map_err(|_| Error::KeyError("Scrypt derivation failed".into()))?;
        ...
        decrypted.extend_from_slice(&aes_decrypt(&[encrypted_part1, &decrypted[0..8]].concat(), derive_half2)?);
        ...
        private_key = private_key.mul_tweak(&factor_b_key.into())
...
pub fn encrypt_private_key(
    ...
    data.push(if compressed { BIP38_FLAG_COMPRESSED } else { 0x00 });
...
pub fn generate_intermediate_code(password: &str, lot: Option<u32>, sequence: Option<u32>) -> Result<String> {
    use rand::Rng;
...
impl Bip38Builder {
    ...
    pub fn encrypt(&self, private_key: &SecretKey) -> Result<Bip38EncryptedKey> {
        let password = self.password.as_ref()
            .ok_or(Error::InvalidParameter("Password required".into()))?;
...
mod tests {
    ...
    const TEST_VECTOR_1_ENCRYPTED: &str = "6PRVWUbkzzsbcVac2qwfssoUJAN1Xhrg6bNk8J7Nzm5H7kxEbn2Nh2ZoGg";
    ...
    const TEST_VECTOR_2_ENCRYPTED: &str = "6PRNFFkZc2NZ6dJqFfhRoFNMR9Lnyj7dYGrzdgXXVMXcxoKTePPX1dWByq";
    ...
    fn test_bip38_compressed_uncompressed() {
        let private_key = SecretKey::from_slice(&[
            0x64, 0x4D, 0xC7, 0x6B, 0x88, 0xDF, 0x64, 0xC3,
            0xE4, 0x8A, 0xB6, 0x59, 0x5C, 0xBB, 0x5C, 0x46,
            0x8D, 0x63, 0xF2, 0x0B, 0x5C, 0x8D, 0x17, 0x39,
            0xB1, 0x5A, 0x8C, 0x3D, 0x7F, 0xC9, 0x77, 0x0C,
        ]).unwrap();
        
        let password = "TestPassword";
        
        // Test uncompressed
        let uncompressed = encrypt_private_key(
            &private_key,
            password,
            false,
            Network::Dash,
        ...
        // Test compressed
        let compressed = encrypt_private_key(
            &private_key,
            password,
            true,
            Network::Dash,
    ...
    fn test_intermediate_code_generation() {
        ...
        // Intermediate codes start with "passphrase"
        assert!(intermediate.starts_with("passphrase"));
        
        // Test with lot/sequence
        let intermediate_lot = generate_intermediate_code("password", Some(100000), Some(1)).unwrap();
        assert!(intermediate_lot.starts_with("passphrase"));
    }
