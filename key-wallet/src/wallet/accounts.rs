...
impl Wallet {
    ...
    pub fn add_account(
        ...
        let account = Account::new(Some(wallet_id), typed_account.clone(), account_key, network)?;
        ...
        let account_exists = match &typed_account {
            AccountType::CoinJoin {
                ..
            } => collection.coinjoin_accounts.contains_key(&index),
            AccountType::Standard {
                standard_account_type,
                ..
            } => match standard_account_type {
                StandardAccountType::BIP44Account => {
                    collection.standard_bip44_accounts.contains_key(&index)
                }
                StandardAccountType::BIP32Account => {
                    collection.standard_bip32_accounts.contains_key(&index)
                }
            },
            _ => false,
        };
    ...
    pub fn add_special_account(
        ...
        match &account_type {
            ...
            _ => Err(Error::InvalidParameter(format!(
                "Account type {:?} is not a special account type",
                account_type
            ))),
        }
    }
