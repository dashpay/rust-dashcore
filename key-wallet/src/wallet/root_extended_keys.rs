...
impl RootExtendedPrivKey {
    ...
    pub fn to_bls_extended_priv_key(&self, network: Network) -> Result<ExtendedBLSPrivKey, Error> {
        ...
        // The scalar is read little-endian from the secp secret, as it always
        // was; the bag holds big-endian, so the bytes are reversed going in.
        let mut scalar_bytes = self.root_private_key.secret_bytes();
