...
fn test_bip44_account_creation() {
    ...
    for index in 0..10 {
        ...
        match &account.account_type {
            AccountType::Standard { index: acc_index, standard_account_type } => {
...
fn test_coinjoin_account_creation() {
    ...
    for index in 0..3 {
        let account_type = AccountType::CoinJoin { index };
        ...
        // Verify account properties
        match &account.account_type {
            AccountType::CoinJoin { index: acc_index } => {
                assert_eq!(*acc_index, index);
            }
...
fn test_identity_topup_account() {
    ...
    for registration_index in 0..3 {
        ...
        // Verify account properties
        match &account.account_type {
            AccountType::IdentityTopUp { registration_index: reg_idx } => {
...
fn test_account_derivation_path_uniqueness() {
    ...
    let account_types = vec![
        (AccountType::Standard { index: 0, standard_account_type: StandardAccountType::BIP44Account }, "m/44'/1'/0'".to_string()),
        (AccountType::Standard { index: 1, standard_account_type: StandardAccountType::BIP44Account }, "m/44'/1'/1'".to_string()),
        (AccountType::Standard { index: 0, standard_account_type: StandardAccountType::BIP32Account }, "m/0'".to_string()),
        (AccountType::CoinJoin { index: 0 }, "m/9'/1'/0'".to_string()),
        (AccountType::IdentityRegistration, "m/9'/1'/5'/1'".to_string()),
        (AccountType::IdentityTopUp { registration_index: 0 }, "m/9'/1'/5'/2'/0'".to_string()),
