...
fn test_transaction_metadata_storage() {
    ...
    // Query by category
    let income_txs: Vec<_> = metadata_store
        .values()
    ...
    // Query by tag
    let test_tagged: Vec<_> = metadata_store
        .values()
        .filter(|m| m.tags.contains(&"test".to_string()))
        .collect();
...
fn test_corrupted_transaction_recovery() {
    ...
    let test_cases = vec![
        (vec![], vec![TxOut { value: 1000, script_pubkey: ScriptBuf::new() }], TransactionError::InvalidInput),
        (vec![TxIn { 
            previous_output: OutPoint::null(), 
            script_sig: ScriptBuf::new(), 
            sequence: 0, 
            witness: dashcore::Witness::default() 
        }], vec![], TransactionError::InvalidOutput),
    ];
...
fn test_transaction_fee_estimation() {
    ...
    for (inputs, outputs, expected_size) in test_cases {
        ...
        assert!(
            ...
            estimated, expected_size, margin
        );
    }
}
...
fn test_transaction_replacement_by_fee() {
    ...
    assert_eq!(
        original_tx.input[0].previous_output,
        replacement_tx.input[0].previous_output
    );
}
