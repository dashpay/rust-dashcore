...
fn test_wallet_mnemonic_export() {
    ...
    match &wallet.wallet_type {
        WalletType::Mnemonic { mnemonic: exported, .. } => {
...
fn test_wallet_full_backup_restore() {
    ...
    let mnemonic = match &original_wallet.wallet_type {
        WalletType::Mnemonic { mnemonic, .. } => mnemonic.clone(),
        _ => panic!("Expected mnemonic wallet"),
    };
...
fn test_wallet_partial_backup() {
    ...
    let account_metadata = vec![
        (1, AccountType::Standard { index: 1, standard_account_type: StandardAccountType::BIP44Account }),
        (0, AccountType::CoinJoin { index: 0 }),
    ];
...
fn test_wallet_metadata_backup() {
    ...
    let metadata = vec![
        ...
        AccountMetadata {
            account_type: AccountType::CoinJoin { index: 0 },
...
fn test_incremental_backup() {
    ...
    let initial_account_count = wallet.accounts
        .get(&Network::Testnet)
    ...
    let new_account_count = wallet.accounts
        .get(&Network::Testnet)
        .map(|c| c.standard_bip44_accounts.len())
        .unwrap_or(0);
    ...
    let coinjoin_count = wallet.accounts
        .get(&Network::Testnet)
        .map(|c| c.coinjoin_accounts.len())
        .unwrap_or(0);
