...
use crate::account::types::{ManagedAccountType, StandardAccountType as ManagedStandardAccountType};
...
fn create_test_managed_account(network: Network, account_type: AccountType) -> ManagedAccount {
    ...
    match account_type {
        ...
        AccountType::IdentityRegistration => {
            let addresses = AddressPool::new(base_path, false, 20, network);
            let managed_type = ManagedAccountType::IdentityRegistration { addresses };
            ManagedAccount::new(managed_type, network, GapLimitManager::default(), false)
        }
        AccountType::IdentityTopUp { registration_index } => {
            let addresses = AddressPool::new(base_path, false, 20, network);
            let managed_type = ManagedAccountType::IdentityTopUp { registration_index, addresses };
            ManagedAccount::new(managed_type, network, GapLimitManager::default(), false)
        }
        AccountType::IdentityTopUpNotBoundToIdentity => {
            let addresses = AddressPool::new(base_path, false, 20, network);
            let managed_type = ManagedAccountType::IdentityTopUpNotBoundToIdentity { addresses };
            ManagedAccount::new(managed_type, network, GapLimitManager::default(), false)
        }
        AccountType::IdentityInvitation => {
            let addresses = AddressPool::new(base_path, false, 20, network);
            let managed_type = ManagedAccountType::IdentityInvitation { addresses };
            ManagedAccount::new(managed_type, network, GapLimitManager::default(), false)
        }
        AccountType::ProviderVotingKeys => {
            let addresses = AddressPool::new(base_path, false, 20, network);
            let managed_type = ManagedAccountType::ProviderVotingKeys { addresses };
            ManagedAccount::new(managed_type, network, GapLimitManager::default(), false)
        }
        AccountType::ProviderOwnerKeys => {
            let addresses = AddressPool::new(base_path, false, 20, network);
            let managed_type = ManagedAccountType::ProviderOwnerKeys { addresses };
            ManagedAccount::new(managed_type, network, GapLimitManager::default(), false)
        }
        AccountType::ProviderOperatorKeys => {
            let addresses = AddressPool::new(base_path, false, 20, network);
            let managed_type = ManagedAccountType::ProviderOperatorKeys { addresses };
            ManagedAccount::new(managed_type, network, GapLimitManager::default(), false)
        }
        AccountType::ProviderPlatformKeys => {
            let addresses = AddressPool::new(base_path, false, 20, network);
            let managed_type = ManagedAccountType::ProviderPlatformKeys { addresses };
            ManagedAccount::new(managed_type, network, GapLimitManager::default(), false)
        }
    }
}
...
fn test_transaction_routing_to_coinjoin_account() {
    ...
    let account_type = AccountType::CoinJoin { index: 0 };
    let managed_account = create_test_managed_account(network, account_type);
