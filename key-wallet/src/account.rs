...
#[cfg_attr(feature = "serde", derive(Serialize, Deserialize, Encode, Decode))]
...
impl Account {
    ...
    pub fn new(
        ...
        let external_path = DerivationPath::from(vec![ChildNumber::from_normal_idx(0).map_err(Error::Bip32)?]);
        let internal_path = DerivationPath::from(vec![ChildNumber::from_normal_idx(1).map_err(Error::Bip32)?]);
    ...
    pub fn enable_coinjoin(&mut self, gap_limit: u32) -> Result<()> {
        ...
        let mut coinjoin_external = AddressPool::new(
            coinjoin_external_path.clone(),
            false,
            gap_limit,
            self.network,
        );
        let mut coinjoin_internal = AddressPool::new(
            coinjoin_internal_path.clone(),
            true,
            gap_limit,
            self.network,
        );
    ...
    pub fn get_next_coinjoin_receive_address(&mut self) -> Result<Address> {
        let key_source = self.get_key_source()?;
        let pools = self.coinjoin_addresses.as_mut()
            .ok_or(Error::CoinJoinNotEnabled)?;
    ...
    pub fn mark_address_used(&mut self, address: &Address) -> bool {
        ...
        if self.external_addresses.mark_used(address) {
            self.gap_limits.external.mark_used(
                self.external_addresses.get_address_index(address).unwrap_or(0)
            );
        ...
        if self.internal_addresses.mark_used(address) {
            self.gap_limits.internal.mark_used(
                self.internal_addresses.get_address_index(address).unwrap_or(0)
            );
            self.metadata.last_used = Some(Self::current_timestamp());
            return true;
        }
    ...
    pub fn contains_address(&self, address: &Address) -> bool {
        self.external_addresses.contains_address(address)
            ...
            || self.coinjoin_addresses.as_ref()
                .map(|cj| cj.external.contains_address(address) || cj.internal.contains_address(address))
                .unwrap_or(false)
    }
...
mod tests {
    ...
    fn test_serialization() {
        ...
        assert_eq!(
            account.get_all_addresses().len(),
            deserialized.get_all_addresses().len()
        );
    }
}