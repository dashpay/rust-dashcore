...
impl WatchOnlyWallet {
    ...
    pub fn new(xpub: ExtendedPubKey, network: Network, name: String, index: u32) -> Result<Self> {
        // The xpub is already at the account level (e.g., m/44'/5'/0')
        // We can only derive non-hardened paths from here
        let account_path = DerivationPath::from(vec![]);

        // Create external path (0) - relative to the account xpub
        let external_path =
            DerivationPath::from(vec![ChildNumber::from_normal_idx(0).map_err(Error::Bip32)?]);

        // Create internal path (1) - relative to the account xpub
        let internal_path =
    ...
    pub fn mark_address_as_used(&mut self, address: &Address) -> bool {
        self.external_pool.mark_used(address) || self.internal_pool.mark_used(address)
    }
    ...
    pub fn owns_address(&self, address: &Address) -> bool {
        self.external_pool.contains_address(address) || self.internal_pool.contains_address(address)
    }
        ...
        F: Fn(&Address) -> bool + Clone,
    {
        ...
        ScanResult {
            ...
            new_external_index: external_stats.highest_generated.map(|h| h + 1).unwrap_or(0),
            new_internal_index: internal_stats.highest_generated.map(|h| h + 1).unwrap_or(0),
        }
    }
    ...
    pub fn get_address_path(&self, address: &Address) -> Option<DerivationPath> {
        if let Some(info) = self.get_address_info(address) {
            ...
            let change = if self.internal_pool.contains_address(address) {
                1
            } else {
                0
            };
...
impl WatchOnlyWalletBuilder {
    ...
    pub fn build(self) -> Result<WatchOnlyWallet> {
        let xpub = self
            .xpub
            .ok_or_else(|| Error::InvalidParameter("Extended public key not provided".into()))?;
