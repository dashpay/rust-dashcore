...
impl GapLimitManager {
    ...
    pub fn is_discovery_complete(&self) -> bool {
        let external_complete = self.external.stage == GapLimitStage::Complete 
            || self.external.limit_reached;
        let internal_complete = self.internal.stage == GapLimitStage::Complete 
            || self.internal.limit_reached;
        let coinjoin_complete = self.coinjoin.as_ref()
...
mod tests {
    ...
    fn test_addresses_to_generate() {
        ...
        // After using one
        gap.mark_used(2);
        assert_eq!(gap.addresses_to_generate(), 3); // Need to generate 5, 6, 7 to maintain gap
    }
}