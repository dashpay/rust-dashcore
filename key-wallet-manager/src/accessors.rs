...
mod tests {
    ...
    #[test]
    fn get_wallet_and_info_mut_returns_none_then_some() {
        let mut manager: WalletManager<ManagedWalletInfo> = WalletManager::new(Network::Testnet);
        let wallet = build_wallet();
        let id = wallet.compute_wallet_id();

        assert!(manager.get_wallet_and_info_mut(&id).is_none());

        let info = ManagedWalletInfo::from_wallet(&wallet);
        manager.insert_wallet(wallet, info).expect("insert");

        let (_wallet_ref, info_mut) =
            manager.get_wallet_and_info_mut(&id).expect("present after insert");
        info_mut.set_name("renamed".to_string());

        assert_eq!(manager.get_wallet_info(&id).unwrap().name(), Some("renamed"));
    }

    #[test]
    fn get_wallet_mut_and_info_mut_allows_split_borrow() {
        let mut manager: WalletManager<ManagedWalletInfo> = WalletManager::new(Network::Testnet);
        let wallet = build_wallet();
        let info = ManagedWalletInfo::from_wallet(&wallet);
        let id = manager.insert_wallet(wallet, info).expect("insert");

        // Compile-time proof that &mut Wallet and &mut T coexist: call a
        // `&mut self` method on each within the same borrow scope.
        let (wallet_mut, info_mut) = manager.get_wallet_mut_and_info_mut(&id).expect("present");
        let _account = wallet_mut.get_bip44_account_mut(0);
        info_mut.set_name("touched".to_string());

        assert_eq!(manager.get_wallet_info(&id).unwrap().name(), Some("touched"));
    }

    #[test]
    fn insert_wallet_bumps_monitor_revision() {
        let mut manager: WalletManager<ManagedWalletInfo> = WalletManager::new(Network::Testnet);
        let before = manager.monitor_revision();

        let wallet = build_wallet();
        let info = ManagedWalletInfo::from_wallet(&wallet);
        let info_revision = info.monitor_revision();
        manager.insert_wallet(wallet, info).expect("insert");

        let after = manager.monitor_revision();
        // `monitor_revision` is structural_revision + sum of infos' revisions.
        // After insert, the delta is 1 (the structural bump) plus whatever the
        // fresh info's revision contributes — so the delta must be >= 1.
        assert!(
            after > before + info_revision,
            "expected monitor_revision > {} + {}, got {}",
            before,
            info_revision,
            after
        );
