...
pub trait FilterFetcher: Send + Sync {
    ...
    fn fetch_filters(&mut self, block_hashes: &[BlockHash]) -> Result<Vec<CompactFilter>, FetchError> {
...
impl FilterClient {
    ...
    pub fn update_from_wallet_manager(&mut self, manager: &EnhancedWalletManager) {
        ...
        // Add all wallet scripts
        for wallet in manager.wallets.values() {
            for address in &wallet.monitored_addresses {
                let script = ScriptBuf::from(address.script_pubkey());
                self.watched_scripts.insert(script);
            }
            
            // Add all wallet UTXOs as watched outpoints
            for utxo in wallet.utxo_set.all() {
                self.watched_outpoints.insert(utxo.outpoint);
            }
        }
    }
    ...
    pub fn process_filter(
        ...
        // Cache the filter
        self.filter_chain.cache_filter(filter.clone());
    ...
    pub fn fetch_and_process_block(
        ...
        let fetcher = self.block_fetcher.as_mut()
    ...
    pub async fn sync_filters(
        ...
        let fetcher = self.filter_fetcher.as_mut()
            .ok_or(SyncError::NoFilterFetcher)?;
        ...
        for (height, block_hash) in block_hashes {
            ...
            // Fetch the filter
            let filter = fetcher.fetch_filter(&block_hash)
            ...
            if let FilterMatchResult::Match { .. } = match_result {
                sync_result.blocks_matched += 1;
                
                // Fetch and process the full block
                let block_result = self.fetch_and_process_block(&block_hash, height)
                    .map_err(|e| SyncError::FetchError(e))?;
...
impl FilterSPVClient {
    ...
    pub fn new(network: Network) -> Self {
        Self {
            ...
            wallet_manager: EnhancedWalletManager::new(network, 20, 100),
    ...
    pub fn add_wallet(
        ...
        self.wallet_manager.create_wallet_from_mnemonic(
            wallet_id,
            name,
            mnemonic,
            passphrase,
    ...
    pub fn process_new_filter(
        ...
        match match_result {
            FilterMatchResult::Match { .. } => {
                ...
                let block_result = self.filter_client
                ...
                for tx in &block_result.relevant_txs {
                    self.wallet_manager.receive_transaction(
                        tx,
                        Some(height),
                        Some(block_hash),
                    );
    ...
    pub async fn start_sync(&mut self, from_height: u32) -> Result<SyncResult, String> {
        ...
        let block_hashes: Vec<(u32, BlockHash)> = self.header_chain
            .iter()
            .filter(|(&h, _)| h >= from_height)
            .map(|(&h, &hash)| (h, hash))
            .collect();
        
        let result = self.filter_client
