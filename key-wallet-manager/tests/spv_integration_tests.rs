...
async fn test_block_processing() {
    ...
    let result = manager.process_block(&block, 100).await.unwrap();
...
async fn test_height_updated_after_block_processing() {
    ...
    for height in [1000, 2000, 3000] {
        ...
        manager.process_block(&block, height).await.unwrap();
...
async fn test_immature_balance_matures_during_block_processing() {
    ...
    manager.process_block(&coinbase_block, coinbase_height).await.unwrap();
    ...
    manager.process_block(&maturity_block, maturity_height).await.unwrap();
...
async fn test_block_rescan_marks_transactions_as_existing() {
    ...
    let result1 = manager.process_block(&block, 100).await.unwrap();
    ...
    let result2 = manager.process_block(&block, 100).await.unwrap();
