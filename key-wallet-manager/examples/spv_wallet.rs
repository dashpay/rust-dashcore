...
fn main() {
    ...
    let mut wallet_manager = EnhancedWalletManager::new(Network::Testnet, 20, 100);
    ...
    match wallet_manager.create_wallet_from_mnemonic(
        ...
        Ok(wallet) => {
            println!("✓ Created wallet: {}", wallet.metadata.name);
            println!("  - Birth height: {}", wallet.birth_height);
            println!("  - Monitored addresses: {}", wallet.monitored_addresses.len());
        }
        Err(e) => {
    ...
    println!("  - Watched scripts: {}", filter_client.watched_scripts.len());
    println!("  - Watched outpoints: {}", filter_client.watched_outpoints.len());
    ...
    match match_result {
        key_wallet_manager::filter_client::FilterMatchResult::Match { height, .. } => {
