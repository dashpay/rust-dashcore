...
mod tests {
    use dash_spv_ffi::*;
    use key_wallet_ffi::{FFIError, FFIWalletManager, wallet_manager::{wallet_manager_wallet_count, wallet_manager_free}};
    ...
    fn test_get_wallet_manager() {
        unsafe {
            ...
            let count = wallet_manager_wallet_count(wallet_manager as *const FFIWalletManager, &mut error as *mut FFIError);
            assert_eq!(count, 0);
